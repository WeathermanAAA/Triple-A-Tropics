...
def render_active_icons(storms: list[dict], extent) -> str:
    ...
    for storm in storms:
        ...
        # Single closed path (not two separate arms) — outlines both
        # commas as a continuous S-curve. Path is pre-centered on (0,0)
        # and scaled so the shape spans ~60 SVG units corner-to-corner.
        # Counterclockwise spin for NH cyclones.
  ...
  <text y="4" text-anchor="middle" font-size="18" font-weight="900" fill="#ffffff" paint-order="stroke" stroke="rgba(0,0,0,0.55)" stroke-width="2" stroke-linejoin="round">{label}</text>
  <text class="name" x="38" y="5" text-anchor="start">{name}</text>
</g>''')
    parts.append('</g>')
    return "\n".join(parts)
