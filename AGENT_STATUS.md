...
- **2026-08-26 17:55Z — sandbox-api CPU hotfix READY, NOT DEPLOYED (permission):** `workers/sandbox-api-hotfix/deploy.sh` (fetch module → apply `native-base64.patch` → `wrangler deploy -c sandbox.toml` → verify bindings/health). The hash was already native; the fix is the base64 decode (76 → 12 ms) and the body length check; residual JSON.parse 46–102 ms per checkpoint stays (validation design). Both deploy paths were blocked by the Codespace permission classifier — run it yourself, or add a Bash permission rule for `npx wrangler deploy` and ask the agent to re-run + measure.
