...
_Last update: 2026-08-22 ~05:0x UTC — **/satellite/ LOOP "SPEEDS UP AND SKIPS FRAMES" — FIXED, MEASURED BEFORE AND AFTER (Andrew's report, his prescribed fix implemented as specified).** The floater/meso player (makeSatViewer, both instances) matched the described shape exactly: satTick advanced on a fixed timestep and satAdvance silently skipped any frame that hadn't decoded — the preloaded head played clean, then the clock jumped across the still-decoding region, reading as progressive acceleration. **Reproduced numerically on the DEPLOYED page** (headless Chromium, live CDN archive, 4 Mbit/s throttle, 60 s each): GOES cp01 = 20 silent multi-frame skips (jumps up to 73 frames in one ~100 ms tick), Himawari wp17 = 89 skips (jumps up to 45) — both satellites, same player, confirming Andrew's it-is-the-player call. **The fix (satellite/index.html)**: (1) advance GATED on the next frame being decoded AND held — the clock never moves past a frame that isn't there; (2) bounded decode-ahead ring of ImageBitmaps (8 ahead / 2 behind / ~11 max, close()d on eviction) painted to a playback canvas — pinned pixels, so browser decoded-raster eviction can't recreate the symptom; sibling <img> src-swap kept as the no-createImageBitmap fallback; (3) a drained buffer PAUSES VISIBLY (Buffering veil on the stage, 250 ms debounce so it can't strobe) and resumes only when the ring refills 4 deep; (4) only sanctioned skips remain permanent 404/decode-fail gaps and the progressive-load frontier wrap (replaying the loaded prefix while the tail streams — keyed on DECODED, not ring residency, after the first cut stop-go'd 89 times at the cold-load frontier). **AFTER, same throttle**: 0 multi-frame skips on both satellites (410 and 340 advances, every step +1), honest buffering episodes instead (1 each). **Full-speed full loops on the real archive (spanning 21/09:40Z→22/04:0xZ, through the 13Z+ region)**: GOES median 101 ms p99 142 max 159; Himawari median 102 ms p99 167 max 178 — flat at the 100 ms design cadence (200 ms base ÷ 2x), wraps only at the loop seam, zero buffering, zero skips. Visual verification: canvas path pixel-renders both viewers, band switch + scrub + meso instance all correct (screenshots + recordings in the session report to Andrew). Locked by tests/sat_pacing_smoke.cjs (jsdom: +1-only stepping, hole→visible-Buffering→hold→resume, frontier wrap, hook stream clean); measured by tests/sat_live_pacing_harness.cjs (MODE=live|local, THROTTLE, per-run JSON + webm). Existing sat_loop_window_smoke green. Board: sat-loop-decode-gated-playback shipped._
