...
_Last update: 2026-07-29 ~17:5x UTC — **ONE SSHWS CATEGORY PALETTE FOR THE WHOLE SITE (eight copies collapsed).** Andrew's reference-image colours adopted VERBATIM — TD `#6eebf9` · TS `#9cf94d` · C1 `#fdfc53` · C2 `#f1af3d` · C3 `#e63222` · C4 `#e732f4` · C5 `#f6c5fb`, thresholds untouched. **The brief said five disagreeing definitions; there were eight.** The three the hex-keyed sweep missed only turned up on a STRUCTURAL re-sweep (any TD..C5 -> colour map, whatever hexes it used): `generate_season_gif.py` carried the same coral/rose ramp as `season_animation.js` (so a storm was one colour in the baked GIF and another in the live animation directly above it on the same page), `tsr/render.py` had a Tailwind-ish ramp for the satellite title-strip badge, and `ascat.js` held a verbatim copy of recon's 15-bin ramp under a comment claiming it was "shared with recon". `tsr/centerfix_plot.py` was the instructive one: it imported `ace_core.SSHS_COLORS` under a comment reading "imported, never re-derived" and kept a hardcoded `except` fallback underneath that had drifted — so any import hiccup drew a whole plate in stale colours with nothing to say so. **ARCHITECTURE**: `palette/tat_palettes/categories.py` is the single definition (pure stdlib on purpose — ace_core and the ACE/tracks/records path run on a deliberately minimal pandas+numpy footprint with no matplotlib, and the palette must cost them nothing). The browser cannot import Python, so `tat_palette.js` / `tat_palette.css` are GENERATED from it and committed; JS reads `window.TATPalette`, CSS reads `var(--cat-c3)`. **The recon/ASCAT fine ramp is now DERIVED rather than hand-tuned**: exactly the category colour at every SSHWS threshold, blending between them for the sub-threshold bins so sub-hurricane winds keep their resolution — which also retired the two hand-tuned "high contrast" copies (that style still differs in basemap/stroke, just not in a second colour ramp). Deleted `TRACKS_JS`, ~500 lines of dead template orphaned by a6785d15 when the live overlay replaced it, carrying one of the stale copies. **GUARDS**: `tests/test_category_palette_ssot.py` fails if a category hex appears literally anywhere outside the palette module (proven non-vacuous — planting `#E63222` in season_animation.js fails it, case-insensitively), if the generated mirror is stale or hand-edited, if the JS classifier disagrees with Python across the threshold boundaries, or if a swatch/ink pair drops below 4:1 contrast. **That contrast guard earned its keep immediately**: white on the new C4 measures **3.41:1**, so C4 chips take the house dark ink now — a forced consequence of the colours, not a style choice. Palette `?v=` stamping extended site-wide so a recolour cannot ship half-cached. **ACE GATE PASSED**: wp/al/ep `--no-live` ACE + tracks JSON byte-identical before/after modulo the wall-clock stamps (`generated_utc` / `updated` / `staleness_minutes`) — run with PYTHONPATH forcing the tree, because a stale `ace-core 0.8.6` is pip-installed in this Codespace and would have made the gate vacuous ([[ace-core-install-shadowing]]). 951 tests; only the documented pre-existing failures remain (4 enscenters ECMWF-mirror `ModuleNotFoundError: ecmwf`, tiled-viewer playback). tsr cyclolab/centerfix 168 green; the 5 `test_cyclolab_visual` failures reproduce IDENTICALLY on pristine HEAD (webkit host deps + a chromium cadence flake), so none are mine. **VERIFIED VISUALLY, NOT JUST BY GREP** — every surface driven in Chromium with the CDN proxied server-side (R2 sends no CORS header for localhost, which is why the first pass rendered blank canvases and proved nothing): home map (MapLibre paint spec, WebGL so invisible to a pixel probe), per-basin tracks, ACE chart, season animation (all seven confirmed as exact canvas PIXELS), records explorer + gantt, recon and ASCAT (seven exact pixel colours each), enscenters (C4 obs glyph — DOLPHIN is the only active WPAC storm), and CycloLab rendered locally for DOLPHIN: lockup, track history, wind history, guidance spaghetti and the diagnostic plate's category bands all on the new palette, zero old-ramp hexes anywhere. Versions/tags: `tat-palettes-v0.3.0`, `ace-core-v0.8.9`, `hafs-render-v0.14.0`; tsr repinned and pushed (@6180c1d, relayed via the box — Codespace token is 403 for tat-satellite-render). **QUEUED (Andrew) — ONE BLOCKING STEP, and a hazard worth reading:** the box rebuild is NOT done, deliberately. `/root/tat-satellite-render` is the docker build context for the whole `tat-render` lane and its working tree is **105 files dirty** with someone's in-flight restructuring, including `centerfix_plot.py` and `tests/test_centerfix_plot.py` staged as DELETED and `requirements.txt` edited DOWN to `tat-palettes-v0.2.2` / `ace-core-v0.8.7`. The live `intensity-poller` container currently has ace-core **0.8.8** installed, so **a rebuild from that tree right now would REGRESS the live stack**, not advance it — that dirty tree is silently overriding pin bumps, which means a future "pin bumped + lane rebuilt" line in this log can be true in the repo and false on the box. Land or discard that WIP first, then: `cd /root/tat-satellite-render && git status` (resolve), `git pull --ff-only origin main`, confirm `grep -E "tat-palettes @|ace-core @" requirements.txt` shows v0.3.0 / v0.8.9, then `docker compose -p tat-render -f /root/tsr-s2/docker-compose.render.yml -f /root/tat-satellite-render/docker-compose.render.yml up -d --build`. Until then CycloLab keeps serving the OLD category colours while every Pages surface serves the new ones — the one surface still drifted, and it is drifted for an operational reason, not a code one. HAFS plate PNGs on R2 also still carry the old chip colours until the next `update-hafs` cycle re-renders them (~6 h, no action needed)._
