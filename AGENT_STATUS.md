...
_Last update: 2026-09-21 ~23:3x UTC — **FLOATER STALL TRIAGED BLIND: box1 is STILL under the Sep 1 CPU limiter (steal 86-95 %), the floater has produced almost nothing since Sep 1, and the shared render service + Caddy stopped answering ~05:45Z today; the box is unreachable from the Codespace (22/443 filtered, IP-specific).** Evidence without the box: freshness-monitor run logs (floater frame row stale 475 h; sampled ages 41 h / 92 min / 20 h / 25 days), floaters/backdrops.json last 05:34Z, per-storm manifests holding a single 05:30-05:40Z frame after the 24 h prune, Dujuan active since Sep 13 with no floater manifest at all, WebFetch ECONNREFUSED on render.triple-a-tropics.com:443 from a second network (Caddy down: draw-a-box, explorer live renders, microwave viewer are down too). Storm-position source is current (tracks feed 22:06Z, Fay fix 18:00Z, NHC 21:00Z): not the split. Meso is not a healthy control (4-5 IR frames/day) but fails less because it has two dedicated render containers, a 45 s timeout and small files; the floater shares tat-render's one `render` at 300 s on full-disk crops. Banner: neither sat-health.js variant is showing (probe feeds fresh via box2 conus/ir; jsdom-verified); the panel text is the generic per-panel note; the NOAA variant is date-gated to Jul 15-18 -- no bug. Nothing touched: no restart, no deletion, no .env, no deploy, no sandbox. **Proposed (awaiting the go):** re-arm `.github/workflows/floater-worker.yml` on :07/:37 with riders OFF (diff prepared, not committed); then on the box: read-only diag script staged locally, bring caddy/render back with cause known, deploy the queue deadline (tsr 72cb9d9 / /root/tat-step3-deploy.sh), give the floater its own render container or move it to box2. Andrew: the limiter (hPanel/ticket). Report: `INCIDENT-2026-09-21-floater-stall.md`; roadmap: floater-render-isolation (next), box1-hostinger-cpu-limiter note._

_Last update: 2026-09-02 ~23:3x UTC — **END OF DAY: CAP UNCHANGED AFTER 4 h OF SHEDDING; LIFT-WATCH ARMED; DEPLOY RUNS ITSELF WHEN STEAL < 50 %.** Watch 21:37-23:22Z after the conus-fast2 pause: steal 91.8-94.4 %, idle 0.5-3.3 %, load 4.6-17.5 -- the guest still consumes 100 % of its ~0.4 vCPU with only cyclolab, radar, s1-render, the meso stack (healthy, frames current), the overlay pollers and g19fd-leads running, so Hostinger's "usage drops to normal" auto-lift is not reachable from inside; Andrew notified 22:30Z (desktop only, Remote Control inactive). Nothing further is approved on the box. State on box1: meso-render/-cold restarted 19:18/19:21 (queues gone), tat-s2-g19fd, tat-s2-conus, tat-s2-conus-fast2, tat-s1-s1-ingest-goes18 stopped by hand (restart=always re-arms them on any dockerd restart; fleet.sh deploy re-ups the lanes), the two other s1 lanes still stranded exited, cyclolab/radar/s1-render/pollers untouched. Code on tsr main @72cb9d9 (heartbeat steal% + rx LIVE on both boxes; render queue deadline + s2 governor landed, not deployed; box1 .env keys set). Tonight's remaining levers are Andrew's: hPanel meters + the once-per-week CPU-limit removal (now defensible: the zombie queues are gone and the governor .env keys are in place, but the lanes are still ungoverned until the deploy) and the ticket with the bandwidth-tier questions; the s1 backlog decision (S1_RETAIN_H or purge) before the exited lanes start. Full record: `INCIDENT-2026-09-02-box1-cpu-cap.md`._

_Last update: 2026-09-02 ~21:3x UTC — **2 h ESCALATION EXECUTED: conus-fast2 paused (approved); steal flat 93-95 % through the whole 2 h after Step 1.** Ticks 20:21-21:24Z: 95.1, 94.9, 93.9, 93.1, 92.9, 93.2, 94.95 % (the 20:11 dip to 88.7 % was noise). `docker stop tat-s2-conus-fast2-emit-cron-1` at 21:25:47Z, Exited 21:26:14Z; g19fd-leads is the only s2 emitter on box1 (box2's conus-fast lane carries goes19/conus ir, irbd, truecolor, c02, so the CONUS explorer stays current). Demand left on box1: cyclolab render-1 + caddy, radar, s1-render, meso hot/cold + poller (healthy, ~85 uploads/h), the overlay pollers, g19fd-leads. A second 10-min watch runs to ~23:30Z. If the cap still has not moved by then, the remaining levers are Andrew's (hPanel weekly removal, ticket) -- no further shedding is approved._

_Last update: 2026-09-02 ~20:2x UTC — **STEP 2 CODE LANDED ON tsr MAIN @72cb9d9 (NOT DEPLOYED); STEAL STARTING TO FALL (88.7 % at 20:11Z).** Two changes, each adversarially reviewed and re-fixed before landing: (1) `app.py` **render queue deadline** — `render_slot()` wraps the semaphore acquire in `asyncio.wait_for`, refuses with 503 + Retry-After past `RENDER_QUEUE_WAIT_S` (40 s; compose sets meso 15 s, s1 25 s), probes `request.is_disconnected()` before spending a slot (the reviewer's key point: the deadline alone still renders for a client that hung up while queued), honours `X-Render-Wait-S` only from compose-internal callers; 10 tests, reviewer stress-tested 3.11/3.12 cancellation (no leaked slot). (2) `s2_pyramid_emit.py` **catch-up governor** — fcntl slot files under `/run/tat-emit` (bind-mounted by the base compose) with `S2_SLOTS_RESERVED` slots for leads lanes (must equal the number of leads lanes, the reviewer's S2), a 3 s steal gate for browse passes (never loadavg), `S2_MAX_REBUILD_PER_PASS` counting attempts; fail-open with a loud warning if the mount is missing; runbook section + warning never to canary it on a browse lane (the watchdogs trip on a gated lane). box1 `.env` now carries `S2_SLOTS=3 S2_SLOTS_RESERVED=2 S2_GATE_STEAL_MAX=50` (inert until a recreate); box2 ungoverned until its lanes are classified. Both boxes' tsr checkouts at 72cb9d9, no container touched. **Deploy sequence** (when steal < ~50 %) is in the incident report Step 3 and QUEUED. 1 h reading: steal drifted 94.7 -> 90.5 -> 92.9 -> **88.7 % with 7 % idle at 20:11Z**, load 5.7; meso loops healthy (43 uploads/30 min); fd/ir leads still 5.5 h stale on 0.4 vCPU. conus-fast2 decision at 21:25Z stands unless steal keeps falling._

_Last update: 2026-09-02 ~19:5x UTC — **BOX1 STEP 1 EXECUTED ON ANDREW'S GO (19:14-19:25Z); MESO LOOPS BACK AFTER 34 H; HEARTBEAT NOW PUBLISHES STEAL % + NIC KiB/s ON BOTH BOXES.** Andrew: Step 1 exactly as written, one action at a time with docker ps checks; hold the weekly hPanel removal until steal falls on its own; pausing conus-fast2 approved if steal has not moved in 2 h; Step 2 through normal review; heartbeat must gain steal% + eth0 rx. Done, one at a time: meso-render + meso-render-cold restarted (each `docker restart` failed its kill step with "PID is zombie and can not be killed" while the SIGKILL landed; the container reached exited ~20 s later and needed `docker start` — new rule for starved boxes), `docker stop` tat-s2-g19fd-emit-cron-1, tat-s2-conus-emit-cron-1, tat-s1-s1-ingest-goes18-1; cyclolab/radar/s1-render/pollers untouched. Effect within 15 min: memory used 23.0 -> 12.8 GB, load 24 -> 12, **meso-poller uploading frames again from 19:27Z** (renders 24-62 s, hot queue not re-forming), steal still 94-95 % (the auto-lift is the provider's call; 10-min watch to 21:25Z, conus-fast2 pause at 2 h if unchanged). goes19/fd/ir still 14:30Z on the CDN (leads lane cannot catch up on ~0.4 vCPU). **Step 2 item shipped (reviewed, SHIP-after-fixes applied):** tsr `scripts/heartbeat.sh` @4b4eb83 samples /proc/stat + /proc/net/dev over a measured ~3 s window and publishes steal_pct / cpu_busy_pct / cpu_idle_pct / net_if / rx_kib_s / tx_kib_s / sample_s / run_s (nulls on any unreadable input; a mktemp-failure fallback that would have `rm -rf /tmp` was caught and removed before landing); `tests/test_heartbeat_sample.py` pins the awk (gawk and mawk verified); relayed via box1 to tsr main and installed on both boxes 19:43-19:44Z (box1 steal 95.8 %, run 13 s vs 65 s before Step 1; box2 0.0 %). TAT `fleet/index.html` @415524b4 renders "cpu steal" (hot 10/bad 50), "ingress KiB/s", and relabels load "(capped)" at >= 50 % steal; jsdom smoke over capped/free/old/null/missing heartbeats. Roadmap: fleet-heartbeat-steal shipped. Remaining Step 2 (render queue deadline + meso mem_limit, S1_RETAIN_H, s2 governor) stays queued for review; hPanel/ticket items unchanged in QUEUED._

_Last update: 2026-09-02 ~17:4x UTC — **BOX1 INCIDENT TRIAGE (READ-ONLY): HOSTINGER'S AUTOMATED CPU LIMITER, TRIPPED BY A MONTH-END CATCH-UP BURST — nothing killed, restarted, or deployed; report + step list in `INCIDENT-2026-09-02-box1-cpu-cap.md`.** Hostinger's CPU-limit warning + fleet heartbeat load 16-49 on box1. Verdict: not a runaway, not timer overlap, not a grown lane. Since **Sep 1 07:20Z the guest gets ~0.4 of 8 vCPUs** (steal 94-96 % on every vCPU, flat 33+ h, zero in-guest quota/nice/tc; box2 0.00 %). It engaged in the documented 25 %/hour steps (04:10-07:20 Sep 1) after a **synchronized catch-up burst** (6-7 cores 00:20-04:00 Sep 1, global OOM 01:35) that followed a **35 h host-side ingress throttle at exactly 1280 KiB/s** (Aug 30 13:10 -> Sep 1 00:15, box1 only, lifted at the calendar-month boundary); a 6400 KiB/s tier had been on since Aug 24 20:xx, so the "planned ~3.7-core" Aug 25-29 baseline was itself network-queued. Under the cap: meso-render is completing requests queued **27.7 h** ago (no deadline on the semaphore wait; poller circuit open every minute; meso loops empty ~34 h), SQS holds ~4,570 s1 slots 22-29 h old with two ingest lanes stranded exited (dockerd task-create failures, restart manager gave up), s1-ingest-goes18 self-restarts every 11 min (347), browse lanes emit 3 h-old frames at 3-4/h, s1-render memcg-OOMs 20/day, eight global OOMs total (two BEFORE the incident; victim usually render-1 = cyclolab's origin; coincident cgroup peaks ~40 GB on 32 GB). **Adversarially verified** (3 read-only refuters + critic; 4 agents, 120 tool uses): cap mechanism and throttle trigger stand; corrected: the burst was mostly render-1's unbounded queue + the s1 backlog, not the s2 lanes (bounded windows; the 35 h browse hole was never refilled); Hostinger docs say the limiter **auto-lifts when usage drops** and there is a **once-per-week hPanel removal** — so demand-shedding on the box is the unblocker, the ticket the fallback; per-cgroup CPU figures include stolen time on this kernel (PARAVIRT_TIME_ACCOUNTING unset); a loadavg gate would halt the leads lanes and livelock (gate on steal/idle); an unordered flock semaphore is a priority change in disguise (reserve a slot for leads); the 32 TB premise does not fit (shapers hit at ~20 and ~22-25 TB; box2 did ~24-25 TB unshaped); uhr-poller (~half of box1 ingress) was missing from every budget. **Proposed, awaiting a go (Step 1, one at a time, verify each):** restart meso-render then meso-render-cold (drops the zombie queues; only client is meso-poller), `docker stop` the two browse lanes + s1-ingest-goes18 (pure churn), leave cyclolab/radar/s1-render/pollers alone, judge by `sar -u 1 3` steal falling and fd/ir latest_times advancing, escalate to pausing conus-fast2 if not. Step 2 code (tsr, review first): render-app queue deadline + mem_limit (meso copies live in the forbidden tree — Andrew's call), S1_RETAIN_H knob, s2 governor (flock slots with leads reservation, per-pass rebuild cap, steal-aware gate), heartbeat steal %/rx, month-end ingest brake. Recurrence: ~0.86 TB/day per box hits the shapers around day 23-24 and 29-30 every month; moving lanes to box2 only spreads it. Roadmap: box1-hostinger-cpu-limiter, s1-backlog-retain-window (needs-andrew); box1-catch-up-governor, render-queue-deadline, fleet-heartbeat-steal (next). QUEUED: Hostinger hPanel/ticket items below._

_Last update: 2026-08-25 ~21:4x UTC — **COST AUDIT: THE THREE AUTHORIZED CHANGES ARE LIVE, EACH MEASURED, NOT PROJECTED.** Andrew authorized A/B/C in order (each its own commit) and asked for the measured Class A rate and storage after each. **A — models/hafs retention (TAT 32864d7f, 72a5f7c2):** `scripts/hafs_r2_prune.py` + `prune-hafs.yml` (daily 05:31Z, dispatch = dry run; a dispatched dry run proved the workflow with the repo's R2 secrets). Dry-run list recorded in COST-AUDIT-2026-08.md BEFORE deleting; apply from box2 16:13–16:46Z: **168 cycles / 220,745 objects / 435.2 GB deleted, 0 errors**; R2 storage **2,384 → 1,953 GB (−431 GB, −18%)** within the hour, objects 1.789 M → 1.570 M; the prefix is now capped at 14 days by cycle time, so the +$19/month-per-month compounding is over. **B — hwfd sweep cadence (tsr c3f739b):** `sleep 30` → `sleep 900` (knob `S2_HWFD_SWEEP_SLEEP_S`), lane recreated on box2 16:12Z. Measured steady state: sweep every ~23.5 min instead of ~2.8, **~1,350 passes/day vs 11,415, ~14K list pages/day vs 148.7K, hwfd Class A ~21.5K/day vs 161.6K (−140K/day ≈ −$19/month; estimate was $10–15)**; no 20-min slot skipped, ~+12 min mean latency on the 22 browse bands. Fleet hourly Class A fell 21.8K/h (15Z) → 13.6K/h (17Z), LIST/h 15.6K → 6.6K. **C — s2 coverage from the manifest (tsr c7992bf..a6e4500):** built under a 3-lens adversarial review that found two REAL high-severity defects in the naive version (a geometry migration inside the window would flip-flop the live manifest every pass; a frame deleted under a live manifest would never re-render, which the container-canary watchdog's rollback relies on) — fixed with a 30-min verify listing per product (`S2_COVER_VERIFY_S`) and a stray-geometry guard in `emit_one`; an independent re-review of the final code found no blockers and two one-line hardenings (landed). Gates as promised: read-only equivalence tool inside the image → **EQUIVALENT** (manifest `get=1` vs listing 7 pages per product); one-lane canary on box2 hwfd-leads with an auto-rollback watchdog (`scripts/s2_cov_canary_watchdog.sh`) → **PASS 20:45Z** (268 passes, 0 fallbacks/FAILs/tracebacks/strays, −84% list pages on that lane); then `fleet.sh deploy box2` 20:50Z and box1 21:35Z (box1's foreground deploy hit a 15-min timeout mid-build — re-run as a detached unit; consistent state throughout). Rollback lever: `S2_COVER_FROM_MANIFEST=0` in the box `.env` + recreate. Honest projection: ≈ −151K list pages/day (≈ −$21/month), below the −$45 estimate because B took the biggest slice first and the verify cadence is the price of the two fixes; measured fleet numbers land in the audit doc at +1 h/+3 h. **Breaker:** live alert-only since 15:55Z, every tick `ok` all day (rate_1h 13–24K vs trip 150K); `breaker-liveness` runs green. **Workers Paid (Andrew's upgrade ~19:30Z):** sandbox-api `exceededResources` did NOT drop at first — every kill was `exceededCpu` at exactly 50 ms on `PUT /api/checkpoint` / `POST /api/season` while 341 ms successes coexisted — then fell to **0% from 20:10Z** once the isolates recycled (hourly 55 → 47 → 35 → 8 → 1%); residual single-digit kills to watch. **Not done, on purpose:** the box-side kill switch (tsr `cost-breaker` 3b9b02c, TAT `cost-killswitch` cc02f38b) stays merged-nowhere — Andrew's authorization covered A/B/C, and merging TAT's copy to main auto-deploys to the overlay pollers. The concurrent audit session and I coordinated on that. Memory: `s2-coverage-from-manifest`, `r2-storage-hafs-unbounded` updated._

---

_Last update: 2026-08-25 ~16:0x UTC — **COST AUDIT + LOCKDOWN: PHASE 0 BREAKER LIVE (ALERT-ONLY), PHASE 1 MEASURED, PHASE 2 RECOMMENDED — no production change beyond the observe-only Worker.** Box access confirmed first (both boxes SSH-reachable). **Phase 1 (`COST-AUDIT-2026-08.md`, every number measured):** R2 is **2.375 TB / 1.78 M objects** (full bucket walk from box2, 1,782 pages), not 2.8; `shadow/sat` 1.42 TB is FLAT (14-day prune deletes ~100 GB/day; nothing older than 30 d) and **`models/hafs` 0.92 TB has NO retention** — cycles land under `models/hafs/{cycle}/` and never age out, 30–55 GB/day, i.e. 100% of the +41 GB/day storage growth (+$19/month every month). **s2 emit lanes are 98% of all LISTs** (378K of 385K in the same 24 h window, lane `[ops]` counters vs GraphQL); hwfd alone 39% (11,415 passes/day for 22 bands); the per-stamp `_ready.json` probe in `_covered_times → complete_stamps` is the convertible loop, heal/prune/cold-start genuinely need to list. D1 `tat-sandbox` 375.6 MB, **154 users** (not 106), 62,562 storms — fits a box trivially. **Workers are on the FREE plan** (`tat-sandbox-api` cpuP50 = exactly 10 ms, 57,685 `exceededResources` in 3 days = 41% of its requests; ~80K of the 100K req/day cap used). Hostinger KVM 8 = 32 TB/month then 10 Mbps; both boxes at ~74% from inbound ingest alone — serving media from a box is out regardless of disk. **Phase 2 (recommendation only):** accounts backend to box1 behind the existing cloudflared tunnel (agreed, the real exposure is the free-plan caps), media stays on R2 with ceilings, order A–E: hafs 14-day retention → hwfd cadence → manifest-based coverage → accounts migration → scoped analytics token. **Phase 0 (deployed):** `workers/r2-breaker.js` cron (2 min past each 5-min boundary) reads GraphQL Class-A ops, trips on 150K/h (or 300K/h 15-min pace) for 2 ticks, ALERT-ONLY (GitHub issues, label `breaker`; `writes_enabled` untouched), D1 `tat-breaker`, `/r2-breaker/status` contract v1; first tick 15:57:55Z rate 21,294/h ok. Liveness: Worker self-gap issue + `breaker-liveness.yml` red-run (first run green) + `/fleet/` card (renders unreachable as a fault). Built with a 15-agent workflow, three-lens adversarial review, 8 confirmed findings fixed pre-deploy. **Box kill switch built + tested, NOT deployed** (tsr `cost-breaker` 3b9b02c, TAT `cost-killswitch` cc02f38b; 73+65 tests; modules byte-identical): deploying recreates lanes, so it waits for Andrew — see QUEUED. Artifact: https://claude.ai/code/artifact/a0baec50-9074-4d48-a158-043356b9b540 ._

---

_Last update: 2026-08-22 ~16:3x UTC — **SAT LOOP v4 (real speedup fix: slot clock starts at PRESENTATION) + FLOATER LAG DIAGNOSED (report) + a monitor discriminator so it's caught next time.** Andrew: floater still speeding up after v3, plus the floater loop was stuck at 15:00Z while meso was current. **Build check first**: production served v4 (view-source `<meta name="tat-sat-player">` + `state().build`); the crash-build/revert/v2/v3 all deployed earlier — Pages max-age=600 means a tab can lag a deploy ~10 min. **#1 (fixed, deployed, production-verified with the new metric)**: v3 gated the advance but anchored the slot clock to the rAF timestamp the moment it set img.src, so a late callback (previous-frame decode blocking the main thread) shortened the NEXT slot — production showed "painted" slots of 5-23 ms that were never actually visible, the residual speed-up an index gate can't see (and v3's acceptance measured durations "excl. dwell", exactly where it hid). v4: the slot clock starts at PRESENTATION, not callback time — the next frame loads into a second `<img>` (double buffer) at opacity 0 during the current slot and the buffers flip opacity once it reports decoded, so the frame is on screen the instant its clock starts and decode latency is paid inside the prior slot, never added to this one. Every slot, held or painted, then holds the same satStepMs from presentation; time is performance.now() throughout, never the rAF ts. **Acceptance rewritten to measure DISPLAYED slots**: the hook now emits a per-slot `present` event (pixels on screen) and asserts every presented slot >= 0.9x step, held not shorter than painted, dwell ~6x, no long stalls. Production, both satellites: every presented slot >= step (0 short of 256/259), held >= painted, p99 <= 3x median; an independent readout-time capture on the live page showed all displayed-frame steps = 1 (no multi-frame jumps), median 120 ms. jsdom smokes drive a controllable performance.now() and prove a not-yet-presentable slot is never cut short. (The `<canvas>`/ImageBitmap path stays banned — v1's GPU-pinning crash; v4 is pure `<img>` double-buffer.) **#2 (reported; a lag, not a stall)**: the floater loop runs ~22-50 min behind real time, OSCILLATING with its sweep cycle (measured 47-50 at the peak Andrew saw, 22 min at the trough), while meso is ~2 min behind. GLOBAL (R2/CDN, identical for every viewer — not his session), NOT dissipation (LALA active 85 kt; all 9 storms lag together), NOT box CPU (box1 load 0.38/core). Root cause: floater + intensity + guidance pollers share ONE `render` container (`--workers 1`), rate-limited across 9 storms x 6 bands, while meso has its OWN dedicated render container — so floater sweep time (and newest-frame lag) scales with the high storm count and meso doesn't. The precise render-call behavior needs box logs, which are UNREACHABLE from here (SSH egress to the box IPs filtered, gh dispatch 403) → QUEUED manual step (inspect + bump render workers / give floater its own container). **Why the monitor didn't catch it**: the floater freshness rows gate on `floaters/manifest.json` generated_utc, which the poller re-stamps every sweep even with zero new frames (the ACE re-stamped-but-dead blindspot, one product over), AND carried a factually-wrong known_down note ("stalled 2026-07-15"). Cleared the false note and added `floater_frame_lag_rows` (reads the newest FRAME time across active storms; alarms > 90 min = a real freeze, tolerant of the normal 22-50 min sweep oscillation; no suppression; loud on a blind read; tests/test_floater_lag.py, 11 cases). Commits: revert 854c0191, v4 8dd97e7b, discriminator 5bde7c21. Memory: sat-player-imagebitmap-gpu-crash (acceptance lesson appended)._

---

_Last update: 2026-08-22 ~14:2x UTC — **SAT LOOP v3: EVERY ARCHIVE SLOT IS SHOWN OR HELD — AND THE ACCEPTANCE NOW MEASURES PAINTED SLOTS, NOT INDEX STEPS.** Andrew caught that v2's "0 skips" counted index continuity, which by construction cannot see a frame that was never in the sequence — and v2 had two such classes, both documented by me: 404/decode-fail slots stepped over, and the early frontier wrap. Both look like a jump to a human. **v3**: the play sequence is the archive window itself, slot by slot, wrap only at the true end. A permanently-failed slot is HELD for its full duration — the nearest earlier decoded image stays up, the readout shows the slot time with "· no frame", and a data-gap note appears on the stage after 250 ms ("No frame at HH:MMZ — holding the previous image"); a still-loading slot is waited on under the Buffering veil, including a poll-appended tail (no early wrap). Manual step walks onto gap slots the same way. Build marker `<meta name="tat-sat-player">` + `state().build` so "which build am I on" is a view-source away (Pages serves max-age=600, so a tab can lag a deploy by up to 10 min; the crash build lived ~8 h, the revert ~20 min, v2 from 13:30Z). **Acceptance rewritten** (tests/sat_live_pacing_harness.cjs): the hook now reports key/shownKey/held per paint and the viewer exposes frames(); the harness compares each complete lap's painted slot sequence to the archive window (FAIL on any absent or misordered slot, any decoded frame shown as held, any held slot not showing the previous image, any wrap not at the window end) and can inject real gaps by aborting frame requests (GAP=20,41). jsdom smoke rewritten to the same semantics (held gap + coverage + tail-wait). **Results, real archives**: wp17 87 slots and cp01 88 slots, 2 laps each, every slot painted in order, both injected gaps held with the previous image; under 4 Mbit/s throttle every slot painted/held, durations median 100 ms p99 117-118 (flat). GPU-accelerated path, 4 full laps each: no crash, GPU-process peak 518 (wp17) / 523 MB (cp01) and plateaued, renderer under 690 MB. Screenshot of a held gap slot with the note sent to Andrew._

---

_Last update: 2026-08-22 ~13:3x UTC — **SAT LOOP FIX v1 CRASHED PRODUCTION CHROME AT THE LOOP SEAM ("Aw, Snap" error 11) — ROLLED BACK IN MINUTES, CAUSE MEASURED, v2 SHIPPED WITHOUT THE CANVAS.** Andrew reported the 05Z decode-gated player (bdd5e822) killing the renderer at the end of every loop. Rollback first: clean single-commit revert (854c0191), deployed and confirmed before any investigation. **The manifesto I had overridden**: `git log -S` shows it came in with 0bb1a5dd (06-11) after 8c5f2c15 → c28dd9e1 → 2de7f2c5 the same week — every entry is about STUTTER/JUDDER (decode-on-tick LRU p95 ~780 ms; canvas drawImage per-frame GPU re-composite juddering on Retina), never a crash or memory — so it was guarding against jank, and turned out to be right for a reason nobody had measured. **Measured, not inferred** (per-process RSS sampler across full laps; software raster path vs GPU path = chromium channel + accelerated-2d-canvas + ANGLE/swiftshader, dpr 2): software path — renderer flat 662-679 MB, JS heap 38→34, ring capped at 11, 4 laps both satellites, no crash (why the pre-ship headless verification missed it). GPU path — **GPU-process memory climbs +4.3 MB per frame DRAWN (= one 1056×1056 RGBA texture), monotonic, 265 → 539 MB across ONE lap, renderer dies at frame 83/86 = the seam**, exactly Andrew's symptom; renderer RSS and JS heap flat the whole time (the JS-side ring WAS bounded; `close()` freed CPU memory but not the GPU copy). Isolation matrix on the GPU path: no-close variant crashes identically (close() irrelevant); control = the reverted dumb `<img>` player survives 4 laps, GPU climbs to 513 but PLATEAUS and evicts (compositor discardable cache); canvas drawing HTMLImageElements survives too (403 MB); gate-only `<img>` variant survives (527 plateau). **Only ImageBitmap pins textures outside the discardable cache.** Frame dims are uniform 1056² (no seam realloc); Andrew's index-keyed-eviction candidate was moot (ring keyed by src, bounded) — the substance of his hypothesis (monotonic climb to the seam) held, in the GPU process. **v2 forward fix**: the gated advance + play sequence + visible Buffering veil unchanged on the `<img>` src-swap path; decode-ahead buffer = `HTMLImageElement.decode()` warming the next 8 frames of the play sequence (re-decodes an evicted raster before the clock reaches it; the gate waits on the promise; resume at 4 deep); no canvas, no createImageBitmap — the manifesto stays, now citing the measurement. **Acceptance = full loops end-to-end on the GPU path with memory measured**: wp17 4 laps GPU 236→509 peak→488 (evicting), renderer 676 flat; cp01 4 laps GPU 241→495→420, renderer 674; no crash either. Pacing under the bug's 4 Mbit/s throttle: wp17 275/275 +1 steps, cp01 459/459, zero multi-frame skips, 3 and 2 honest buffering holds. jsdom smokes (window + pacing) green after teaching them to yield a task between synchronous ticks (real browsers always do; decode() promises need it). Memory: sat-player-imagebitmap-gpu-crash. Board: sat-loop-decode-gated-playback re-shipped as v2._

---

_Last update: 2026-08-07 ~18:4x UTC — **THE SQS FREE-TIER ALERT WAS A 34-DAY OUTAGE WEARING A BILLING COSTUME — found, fixed, and the whole ingest hardened same-day (Andrew-approved plan, all six items).** The alert's 893k requests unmasked the undocumented tat-sat-ingest AWS account (532918216657): three NOAA NODD SNS→SQS queues feeding box1's S1 shadow ingest. The real finding: **GOES-18 + Himawari-9's SNS subscriptions died upstream 2026-07-05 in the same 5-min window** (queue LastModified==Created — NOAA's drop, not ours) and both lanes ran 100% backfill for 34 days while reporting `healthy: true`, because an empty long-poll counted as health — the 4th success-on-empty instance in two weeks. Shipped, in order: **(1) R2-strict credential guards both repos** — 3 constructors reached real AWS S3 with the live AKIA key from the codespace, 13 more would ship it to Cloudflare via `or AWS_*` fallbacks or boto3's default chain; all now hard-require R2_* (TAT 776e2f7b, tsr 12d21b5; + .env gitignore gap). **(2) Delivery-silence health gate** (tsr 686b860): fresh polls + zero deliveries for 6h ⇒ `"starved"`/healthy:false/503 — reporting only, the watchdog stays poll-gated so a quiet upstream can't restart-loop (tests incl. re-anchor + failing-precedence). **(3) Subscriptions re-created** via the idempotent IaC — GOES-18 delivering in <2 min, `src=sqs` publishes on all three lanes (Himawari's first in 34 days at 18:04Z, completeness-gated full scan). **(4) Band-tight SNS wildcard filters** (tsr e34e676, checker taught the wildcard operator FIRST — it would have silently False'd and blinded acceptance part C; ground truth is now `s1_sources.is_ours`, which promptly exposed that the captured firehose fixture held zero C13): goes19 **160→5 msgs/5min measured** (exactly the C13 cadence, ~97% cut; ~143k→~15-20k req/day expected — CloudWatch tomorrow is the confirmation). **(5) DLQ probe 30s→300s** (tsr f84f7b2, ~8.6k req/day of reading zero). **(6) The drift-rule violation closed**: the never-merged `s1-multisat-copyplots` branch (7 weeks of production!) merged to main (tsr 2407e0b — cyclolab conflicts resolved to main's evolved copy-plot; +frozen-clock fix for time-bomb tests that rotted 3 days after authoring), box1's checkout repointed to main and redeployed via systemd-run (all 4 containers healthy, new health fields live), fleet.yml + RUNBOOK-S1-MULTISAT carry the stack, the incident, and why it's not a fleet.sh lane (cmd_deploy is s2-emit-specific). **Success-on-empty sweep (Andrew's ⚠️) — FINAL: 37 CONFIRMED of 49 candidates** (5 finder agents, every candidate adversarially verified; full findings + smallest-fix per item in tsr `HEALTH-SWEEP-2026-08-07.md`, commit 5e774f9 — report-only per instruction, nothing applied). The shape is endemic, not incidental. **14 HIGH**, headliners: meso_poller's `healthy` gates only on extent-discovery (frame-upload starvation invisible; lane consec_fail reported-not-gated; /health also has no serve-time deadman), floater_poller beats its watchdog at loop-top + stamps manifests on all-basins-failed, cyclolab_guidance_poller's documented heartbeat doesn't exist (statuses computed then discarded), the s2 heartbeat counts containers-up while himawari9/gk2a/mtgi1/geo lanes have NO data-advance monitor, freshness_probe's known_down suppression can mask a NEW failure behind a muted one + the WP intensity feed has no discriminator row (al/ep only), recon/subsurface/CRW/velocity-potential stamps re-tick on empty or cached runs. **The convergence that makes this cheap**: a majority of TAT-side fixes are one-line freshness_probe extractor swaps — rows gate on `generated_utc`-style RENDER stamps while the same JSON already carries an honest DATA field (`date`, `windows.30d.end`, `clips[*].last_frame`, `as_of`, min-across-members); the worker-side set is ~5 small real changes. **12 refutations prove the standing defenses work where they exist**: the intensity/ACE b-deck candidates are covered by the 08-04 btk-discriminator (exactly as designed), two s2 emit candidates died in 52903c7, HAFS starvation is bounded by ens_watchdog + the dead-man render gate. Reported to Andrew with meso_poller + the unmonitored s2 lanes as suggested first fixes. Memories: aws-sqs-satellite-ingest + success-on-empty-health._

---

_Last update: 2026-08-04 ~17:2x UTC — **THE WATCHDOGS EARNED THEIR KEEP ON DAY ONE: two box1 flips auto-rolled-back on a REAL pre-existing bug, now fixed fleet-wide.** Minutes after the conus/fast2 flips, both lanes' watchdogs tripped C1 ("empty index", 2 consecutive rounds) and reverted on their own — flag off, lanes recreated, the offending frames deleted for re-render, verdict files written, zero human involvement. The catch was genuine: six conus products had published marker-complete frames with ZERO tiles at stamp 155617Z (rendered against a still-uploading scan during the lane restarts). That defect PREDATES containers — per-tile mode published the same blank frames invisibly, marker-frozen into the loop for the whole retention window — the container index just made zero countable. **Fix (tsr 340e783): emit_pyramid refuses an all-empty cut in BOTH modes** (emit fails, per-product isolation contains it, the slot re-renders when the scan completes; 77 tests incl. both-mode refusal). Whole fleet rolled onto the guarded image, conus/fast2 re-flipped, their watchdogs re-armed — **all 11 lanes now container-publishing** (mtg auto-flips via its compose switch when the EUMETSAT licence clears), 9 watchdog units active. Also this sweep: the ops sweep gained per-lane log-span divisors (a recreated lane's minutes of log were being diluted over the full window — the first verification run understated box2 by ~20x before the fix)._

---

_Last update: 2026-08-04 ~16:2x UTC — **NEXT-DAY VERIFICATION: THE BILL MOVED. Measured fleet rate 2.1M Class A ops/day vs the 10.7M baseline (−80%) — and the FULL FLEET is now container-publishing behind PASSed canaries.** The scheduled sweep ran on the fleet's own request counters (scripts/s2_ops_sweep.py, per-lane log-span divisors after catching a dilution bug in the first run): box1 1.29M/day (still per-tile at sweep time), box2 848k/day nominal — of which the freshly-flipped lanes' shares are cold-burst extrapolations of minutes-old logs; the CONVERGED flipped signal is conus-fast at **26.8k/day for the lane that cost ~500k/day per-tile**, geo at ~28k settling, mtg 372. **Canary verdict: PASS 11:03Z, 43,200 s watched, zero trips.** **Prune's first complete walk since 07-25: 22,629 stamps / 2,770,876 objects / 97.7 GB deleted across 113 products** (streaming per-product apply after the flat walk OOM'd at 1g and the accumulate-then-apply variant was headed for a second OOM at 2g — both caught and fixed same-night); conus/ir spot-check retains 3,081 stamps ≈ exactly the 14-day window at its mixed cadences. Checks: zero non-mtg FAILs fleet-wide; manifests fresh (conus 8.7 min / geo 14.6 / hwfd 25 on its 20-min lane) and all hinted. **Rollout completed on the PASS**: geo+gk2a flipped 14:56Z, himawari trio 15:35Z, box1's four lanes 16:1xZ — every lane behind its own parameterized watchdog unit (9 active: stale thresholds 7x each lane's step; C1 counts only frames unreadable BOTH ways; the CF-403/UA and transition false-trips from the canary night are all encoded). **Projected steady state once the cold bursts settle: ~0.4-0.6M Class A/day ≈ $2-3/day, from $48/day at the incident peak** — the 15:43 cron's number tomorrow (or scripts/s2_ops_sweep.py --hours 12 on each box any time) is the confirmation, alongside Andrew's billing-page eyeball for 08-03/08-04. mtg stays per-tile-flagged-off until the EUMETSAT licence clears (its first emit will be container-mode automatically via the compose switch when the lane starts succeeding)._

---

_Last update: 2026-08-03 ~23:2x UTC — **CONTAINER CANARY LIVE ON conus-fast, VERIFIED, WITH AUTOMATIC ROLLBACK WATCHING.** Flip at 23:00Z after the baseline gate (all four products fresh, zero FAILs). **Measured write reduction, same pass, same cadence, same pixels**: `goes19-conus-truecolor` rendered **485 tiles at z7 → put=7** (was ~488 objects/frame); `ir` 59 tiles → put=7 — the `[emit] wrote` lines prove identical tile counts and maxzoom, only the object count collapsed (~98.6%). Read path proven on the DEPLOYED site: ranged reads byte-match the block slices exactly (206, three sampled tiles incl. z5, valid WebP), per-tile objects correctly 404, and the headless harness screenshot shows the 22:56Z container frame fully rendered on /satellite/explorer in a 35-frame mixed legacy+container loop. **The watchdog (Andrew's requirement: rollback needs no human)** runs as a restart-on-failure systemd unit checking every 120 s for 12 h: C1 unreadable-frame (index AND per-tile z0 both missing, 2 rounds), C2 ranged-read byte-mismatch/non-WebP (instant), C3 manifest staleness >35 min (TZ-proof timegm), C4 lane container-PUT failures, unreachable-CDN escalation over 5 rounds; trip = flag off + lane recreate + delete container frames within backfill reach so they re-render per-tile + verdict file. **The arming process itself banked three live fixes** — a set -u subshell crash, a Cloudflare 403 on Python-urllib's UA from the box vantage (which false-tripped and thereby PROVED the full automatic rollback end-to-end, twice, before any container frame existed), and C1 transition semantics (a hinted product's newest frame can legitimately still be legacy) — all committed (99cf958, 7774ae5). **Also tonight**: the first prune since 07-25 OOM-died at 1g (whole-tree listing ≈ 50M key strings in RAM); rewritten as a per-product walk (planner + rails byte-unchanged, FakeS3 taught Delimiter, 8/8 tests) and re-running (b118be8). **Next**: watchdog verdict at ~11:00Z (PASS leaves the flag on); the scheduled [ops] sweep sums the fleet against the 10.7M/day baseline — projection ~2.25M/day now, ~0.6M/day once the remaining lanes flip after the canary PASSes. Remaining lanes flip one-per-day-ish, geo/gk2a next (same watchdog, retargeted), then hwfd/g19fd browse; box1 lanes last._

---

_Last update: 2026-08-03 ~19:1x UTC — **R2 CLASS A COST INCIDENT — MEASURED, THEN CUT, SAME SESSION. The bill was 65% LISTING, not writing.** Billing: 98.22M Class A/16 days (~10.7M/day, $445.50 of $448.82), 10× step on 07-25. **Measured decomposition** (8.8 days of lane logs both boxes + run-log forensics + a creds-on-box R2 audit): box tile PUTs ~2.06M/day (box2 1.32M + box1 0.74M), **LIST pages ~7M/day** — the emit loops' own listing: `_covered_times` = 301 LISTs/product/pass (audit-measured exactly), re-listed per slot, manifest rebuild 201/frame, products.json ~55/pass — retention UNPRUNED since the 07-25 restructure (prune-cron never made it into the per-suite lanes; conus products at 2,500–3,465 stamps, 164k total). Plus **the three GH stopgap workflows still scheduled** (conus/geo/diag-riders, ~0.4M/day): checkout ref `s2-sat-ingest` FROZE 2026-07-21 — four days before the delimiter-listing fix — so the third writer nobody counted ran the flat 460k-key walk the whole time; conus runs died at their 65-min timeout mid-LIST with zero output since 08-02, geo spent 45 min/run discovering the boxes already published every slot, and conus's forced `--max-zoom 5 --allow-geometry-change` was a live manifest geometry war against box2's z6/z7 conus products. Andrew's 2a premise (space tiles rewritten 144×/day) checked and REFUTED with numbers: space is alpha-0 and `skip_empty` has always dropped it (FD z5 writes 210 vs ~270 grid); keys are per-stamp so no same-key rewrite exists to hash-skip. **CUTS SHIPPED (resolution + cadence untouched)**: (1) TAT b4adc894 stopgap schedules retired (dispatch kept). (2) tsr 52903c7 ops discipline — tail-bounded coverage listing (StartAfter), in-memory slot re-checks, incremental manifests (one Class B GET + append; full rebuild only cold/geometry/6h-heal), gated+throttled products.json, stall-pin ready-marker HEAD probe, pass-level orphan reconcile, prune-horizon filter, and **exact per-request [ops] counters in every lane log** — a 16-agent adversarial panel confirmed 12 findings pre-deploy (all fixed; 85 s2 tests green; full-suite failures reproduce on clean origin/main). Deployed fleet-wide ~18:30–18:45Z + NEW `tat-s2-prune` lane (box2, 14d TTL, first APPLY walking now). **MEASURED, not projected: gk2a warm pass 1,800→27 list_pages** (cold post-recreate rebuild ~640 by design, then converges); projection ≤3M ops/day total (~$13/day → ~$405/mo saved vs incident rate) — next-day [ops] sweep is the confirmation, Andrew eyeballs the billing page. (3) **Tar-container tile publishing staged** (tsr 629c94a, `S2_CONTAINER_TILES` default OFF): s2_container.py = the hafs #27 pattern zoom-banded for pyramids (~305→~6 objects/FD frame, offsets decode-back-verified), against the remaining ~2.06M PUTs/day (→ ~$270/mo of the bill). **Read path live already** (TAT bb4d88b7): MapLibre `s2c` addProtocol — one 206 Range/tile from CDN-cached blocks, per-frame byte-identical legacy fallback on index 404, NO Worker (Range-on-CORS production-proven by hafs.js on this CDN). Verified: headless boot on live data pixel-matches deployed (after catching my own bug — a hand-typed transparent-tile base64 that errored every skip_empty tile and stalled the readiness-gated preload; ir3d's canvas-encode is the fix), plus a fixture e2e (container+legacy frames in ONE manifest, real 206 serving): container stamp = 0 per-tile fetches / 4 exact-byte ranged reads, both frames render. **Flip is gated** on the running cutb adversarial review (stale-JS-cache transition risk is the open question: old cached tiled_viewer.js can't read container-only frames) — canary one lane after verdicts, rollback = flag off. (c) report: after containers, eager z6/z7 costs ~1.1k PUTs/day (~$0.15/mo) + ~$1/mo storage — on-demand materialization would need publishing 25MB source rasters per frame or a render-on-request service in the hot path to save less than it costs; recommendation NO-BUILD, the container port moots it. **Ops traps hit + memorized**: quiet SSH to boxes wedges (fleet.sh deploy box2 hung AFTER its build finished — lanes recreated by hand; use systemd-run for >2min box jobs), pkill -f self-match killed its own session, and a piped `test | tail` masked a roadmap.yml validation failure into a pushed red-banner board (fixed in 46fe56fb within ~2 min). Session crons: one-shot next-day [ops] verification armed (+ durable instructions in the r2-class-a-incident memory)._

---

_Last update: 2026-08-03 ~06:1x UTC — **GLOBAL ACE HEADER RECONCILED (Andrew's 200.59-vs-226.7 report) — a decaying former TC could wear the PTC flag and take its whole season ACE out of the baked header; killed in ace_core 0.8.13.** The three numbers: the home-map header bakes `compute_header_stats` over the composed per-basin tracks feeds at cron time; the LIVE STATUS strip reads the separate per-basin ACE feeds (`feeds/{b}_ace_data.json`, poller-written) at view time — two pipelines, and the strip's 2.647+54.862+169.195 = **226.704 was right**: it is the exact sum of the 20 published per-storm ACE values, on which cron and poller AGREE storm-for-storm (Andrew's ~220 was just pre-Genevieve-closeout). The header's 200.59 = 226.704 − **26.117 = GENEVIEVE exactly** (also −1 named, −1 cat5). Mechanism, fully evidenced: her b-deck gained a 35 kt **LO** fix 2026-08-02 20:32Z → nature "DS" while she was STILL in CurrentStorms (final advisory 39: systemType **POST-TROPICAL CYCLONE**), which satisfied the PTC promotion gate (designated + NHC-listed + DB/DS last nature) — and `compute_header_stats` excluded is_ptc storms from counts AND total_ace on the false premise "a PTC's ace is 0.0 by construction". Bonus trap: `PTC_CLASSIFICATIONS` carried "PTC" as a *potential*-TC synonym, but NHC's CurrentStorms "PTC" means *post*-tropical. The window self-healed at ~04:47Z when CurrentStorms went empty (feeds already show is_ptc:false; next bake reads ~226.70) — but it RE-ARMS on every AL/EP storm that decays under advisories, so: **FIX (ace_core 0.8.12→0.8.13, adversarially reviewed, three lenses, edge findings folded back + test-pinned)**: (1) pre-genesis guard — TC history (TS/SS/MX in the ACE-nature record, or accrued ACE without an explicit NHC "PC" classification) blocks the PTC promotion; judged on ace_nature-first (the presentational field's wind-fallback "TS" would false-positive), ET deliberately excluded (frontal-origin pre-genesis lows carry EX fixes; every real former TC has TS/SS anyway); (2) header total_ace now sums EVERY storm's ledger ACE, flags ignored — genuine invests/PTCs are 0.0 by construction, so a lying flag can never again subtract real ACE, and baked header == ACE strip by construction (proven on the buggy 04:38Z snapshot: fixed code yields 226.704 even with the wrong flag baked in); (3) "PTC" dropped from the potential synonyms. **Gates: --no-live data output byte-identical before/after** (all six feeds, build stamps aside — structurally guaranteed, PTC gate needs CurrentStorms), full suite green minus the KNOWN pre-existing env failures (same set the 08-01 session stash-verified), 9 new tests in test_ptc_activation.py (26 total there). **Andrew's other suspects, all cleared with evidence**: is_six_hourly/tcvitals-minute — refuted, UCAR vitals are 100% :00-synoptic AND off-minute rows are dropped at parse before they can reach the map (zero-ACE-but-mapped is architecturally impossible today); prefer_bdeck — b-decks LAG (proxy chain live, WP deck current to 00Z) but drop only on exact (SID,time) membership, so a lagging deck cannot claim unpublished hours, and since the contenders redesign overlap rows go to resolve_conflicts, not the void; Dolphin 150 kt — in the deck (07-29 18Z, 909 mb), in the feed (peak 150.0), contributes exactly 2.25, storm ACE recomputes to 38.26 EXACT; SHem season boundary — global composes AL+EP+WP only, calendar-2026, on BOTH displays; SHem 2025-26 is out of scope identically, no disagreement possible. **DEPLOY TAIL**: the box poller container pins ace-core and must pick up 0.8.13 — until then the poller repaints old-rule feed headers over corrected baked pages in a future decay window (not worse than today; cron side is fixed on next run since the workflow pip-installs ./ace_core from checkout). Attempting the box bump next; if the rebuild looks risky (build-context drift memory) it goes to QUEUED manual steps instead._

---

_Last update: 2026-08-07 ~16:2x UTC — **TWO CHRONIC FAILURES DIAGNOSED + FIXED, AND THE MONITOR META-QUESTION ANSWERED BOTH WAYS.** **(1) update-analogs — INCIDENT CLOSED on the stated criterion**: cyclolab/manifest.json is FRESH on the CDN (generated_utc 2026-08-07T13:53:18Z, with `wrote`) after 5 consecutive green runs since 03:32Z. The cartopy fix (82ed890f) DID take — zero import errors since — the post-fix 08-05→08-07 failures were a THIRD, different mode: tropycal's IBTrACS source 404ing per-storm on the WP path (upstream flap; success/failure pairs minutes apart; the 07-16 fail-loud-on-0-of-N guard did its job), self-healed 03:32Z. ~3 weeks start to close. **(2) update-tc-records — NEVER SUCCEEDED ON CRON: 0-for-30 since creation 07-23**, and /records/ serves a 07-29 stamp only because that day's run partially synced before dying on a broken pipe. Three modes: DOMINANT = matplotlib/Pillow lazy-imported inside tc_records/explorer.py's density emit, never installed (**the repo's third lazy-import episode** after analogs' shapely + cartopy); since 08-05 = NCEI instant-403 on every IBTrACS fetch in the cron's GH-lagged 17:15-18:20Z band (evidence: byte-identical curl in update-ace succeeds daily at :07 slots; codespace probes 200 at other hours; curl doesn't retry 403); plus the one broken-pipe publish. **FIXES per the escalation rule ("a third episode means pinning the full dependency set")**: BOTH install steps now pin the complete set to the 2026-08-07 green-runner versions (tropycal 1.5, cartopy 0.25.0, shapely 2.1.2, matplotlib 3.11.1, numpy 2.4.6, pandas 3.0.5, pillow 12.3.0, scipy 1.17.1, requests 2.34.2); tc-records crons moved 16:23/16:53 → 04:23/04:53Z with the evidence in the comment. **Pipeline proven end-to-end locally: rc=0, validation gate passed, density overlays render** — publish verification lands with the next 04:23Z cron. **(3) THE MONITOR: both hypothesized failure classes were real, one each.** ANALOGS = the follow-through gap repeating: the probe DOES exit nonzero and re-alarm 6-hourly (4 red monitor runs sit in the last 30), the alarms were visibly red for weeks — and nobody acted, INCLUDING my own sessions ("left alarming deliberately, needs own diagnosis", 08-01, then six days of other priorities). RECORDS = the blind spot: NO probe entry existed for the records product at all — a 16-day total outage was structurally invisible. Entry added (records/v1/meta.json, 30 h limit): the moment this lands, records goes RED (9 days stale) and re-alarms every 6 h until the moved cron publishes — the monitor itself now enforces this repair's close, which is the correct custody handoff. Board: tc-records-pipeline-repair shipped._

---

_Last update: 2026-08-07 ~06:0x UTC — **HERD AUDIT CLEAN (4-way parallel verification) + #25/#7 SHIPPED on the shared polar core.** **THE HERD**: (1) NO duplicate publishes — the 5 completed successes published 5 DISTINCT consecutive cycles (2026080518→0618, the healthy catch-up ladder); the concurrency group serialized perfectly (each job started 3-5 s after the previous ended) and 4 superseded queue slots cancelled with zero runner time; the one failure was GitHub's own "Service Unavailable" resolving actions BEFORE any step — nothing published or pruned. (2) 0.13.3 HELD: zero STALL FORENSICS / stage-deadline / pool-death lines across every log; every ingest 100% (172/172, 258/258×3), every render 0 failed, all shear frames computed. Two runs ran 287/291 min vs 168 for IDENTICAL plans — max silent gap 49 s, i.e. continuous ~3x-slower throughput on contended outage-recovery runners, not a wedge; the salvage machinery was never needed. (3) COST: NO burst — GH drops missed crons rather than queueing, so the outage produced a ~-5,800-op DEFICIT (failed daily media writers) vs a normal ~29.5k GH-driven day; GH is ~4-5% of the ~594k/day baseline (tsr/satellite dominates, unaffected); back to normal cadence as of 04:25Z. Chronic non-outage failures flagged for their own diagnosis: update-tc-records (0 successes both windows), update-analogs (still 6 fails/day). (4) MANIFEST coherent: cycles unique+sorted, lockstep-pruned, blocks cover frames, ranged read byte-exact, values members present, shear live; generated_at 02:47Z. **#25/#7 (hafs_render 0.16.0)**: polar.py EXTRACTED from shear_diag — one storm-centred geometry + ONE ring-mean operator, three consumers; #26's 14 tests pass unchanged on the shared core (the reuse proof). structure_diag: azimuthal profiles (10m/850/700/500, upper u/v converted from raw m/s), RMW, thickness-derived warm core (hydrostatic, dT = g·dZ/(R·ln(850/500)) — no T levels in cache, stated everywhere), quadrant-MAX radii (method stated in code+manifest+plate). Post-ingest pass publishes storms[].radii + storms[].structure (nest vt_max WITH parent vt_max beside it — the caveat as data); 2x2 house-style plate product "structure" (figure registry, panel-flagged so the frontend gates map semantics — geometry affine/readout/shear overlay never apply to a line chart). **VERIFIED EXTERNALLY**: radii vs b-deck (Dolphin, 5 cycles, n=20): mean +33 nm, mean|d| 35, 90% ≤60 nm, asymmetry ordering preserved → geometry right, offset in the expected model-vs-analyst direction. Structure live-smoked on real frames: textbook plate (850 vt > 10m, -25 kt inflow, +5.1C warm core inside RMW); resolution caveat honestly NULL today (nest-parent +0.7-0.9 kt on broad ≤68 kt storms; 13w compact-case checked at its f084 peak) — the plate prints the comparison every frame so a compact major displays it automatically. 8 new structure tests incl. SH mirror sign flip + antimeridian radii at 179.8E; all suites green (hafs_render 101+, harness 52, root OK)._

---

_Last update: 2026-08-03 ~21:3x UTC — **CORRECTION ON THE CLOUDFLARE DIAGNOSIS — Andrew's token inventory was right and my route claim was wrong.** Full re-verification with exact errors, one credential at a time. **What exists here**: exactly ONE Cloudflare credential (env CLOUDFLARE_API_TOKEN, token id 3b5eb373dde48cc57b0fc881038c4a3d, active — matches tat-codespace-deploy, "used today"); no wrangler OAuth, no other CF tokens in any reachable form, so the other two dashboard tokens (Triple-a-Weather, Edit Cloudflare Workers) cannot be tested from this machine — nothing to test them WITH. **Route claim RETRACTED**: GET /zones/{id}/workers/routes with this token lists `triple-a-tropics.com/telemetry/* -> telemetry-collector` — the route attached at the first deploy; wrangler's "falling back to zone-based API endpoint" was informational, and the 405 I read as failure was edge propagation minutes after creation (a later 500 was equally transient). This token READS zones and routes and EDITS routes (it created this one). The page now posts to the same-zone URL `https://triple-a-tropics.com/telemetry/t` (house pattern, first-party); workers.dev stays as a deploy-smoke surface. **The D1/KV/R2 gap is REAL on this token** — re-verified against the harmless LIST endpoints, not just create: `GET /accounts/.../d1/database`, `/storage/kv/namespaces`, `/r2/buckets` all return success:false, errors:[{code 10000, "Authentication error"}] — a capability absence, not a scoping subtlety (reads fail too, so it is not Edit-vs-Read). **No credential request needed**: the relay design works and costs nothing. The ONLY change that would ever be worth making, if Andrew wants the relay collapsed to a direct R2 put: add the EXISTING Triple-a-Weather token (already lists Account.D1 among its permissions) to the Codespace secrets — sharing an existing token, not creating one — proof-of-need being the three list-endpoint errors above. Zero urgency; the relay stands._

---

_Last update: 2026-08-03 ~21:0x UTC — **GATE MET: #28 READOUTS LANDED + TELEMETRY COLLECTOR LIVE (headroom stated). #25/#7 next.** **(1) COLLECTOR (spec #3 server half)**: the codespace CF token deploys workers but cannot bind KV/D1/R2 (verified: auth 10000 on all three; local AWS keys are real AWS, not R2) — so the live design is a bindingless worker at telemetry-collector.coloradoskier2018.workers.dev relaying each batch via repository_dispatch to telemetry-ingest.yml, which holds the R2 creds and writes telemetry/inbox/{day}/; update-guidance compacts into telemetry/summary.json (scripts/compact_telemetry.py, 90-day rolling) — the demand signal that unblocks the hero-set scheduler (#30). The zone route (/telemetry/*) silently failed to attach (token lacks zone-route perms; GH Pages answered 405) → workers.dev URL instead; the Phase-0 client flush was already shipped dormant and is now armed via TAT_TELEMETRY_ENDPOINT. **Headroom**: worker 100k req/day; dispatch ~5k/hr; Actions FREE (public repo); R2 1 write/batch vs 1M/mo tier; binding ceiling = Actions-run noise, revisit ~500 batches/day. Proven: 403 wrong-origin / 204 valid / ingest run SUCCESS 13s / batch on R2. **(2) #28**: render now emits an 8-bit grey VALUE PLANE per frame (fill field cropped to the published bbox, quantized to the quantity registry's vmin/step with a deterministic step-doubling contract; BT got explicit value_range −170..40 since its display range is per-enhancement), packed into the SAME container block as f{fxx}.values.png — zero extra writes, excluded from the per-frame sync. Frontend: hover lat/lon (works on ALL existing cycles from the manifest affine), value readout (ranged member fetch → canvas decode; activates as planes publish, honest lat/lon-only fallback), two-click great-circle ruler (km/nm + bearing, dlon-normalised, Esc). Proven in-browser (52/52 harness): affine exact at axes centre, container-plane decode = 50 kt at synthetic centre, ruler 111 km/degree; writer unit-tested (orientation, NaN=0, step-doubling roundtrip). **NEXT (the actual ask): #25 azimuthal-mean structure + #7 quadrant-max wind radii** — extract shear_diag's polar machinery into shared helpers (ONE azimuthal-mean implementation, per the instruction), tangential/radial/warm-core panels (thickness-derived T-anomaly — nest cache carries no T levels), quadrant-MAX radii from parent 10m wind stated as such everywhere, verified vs b-deck published radii (distribution, n) + nest-vs-parent resolution comparison making the intensity caveat VISIBLE._

---

_Last update: 2026-08-03 ~18:4x UTC — **THE CANONICAL SPEC WAS NEVER LOST — it arrived on 07-25 and lived where no agent could read it. Now on disk at /workspaces/models_page_build_spec.md** (out of repo, gitignored belt; competitor observations) **with the canonical item numbering, and MODELS-SPEC-STATUS.md fully rewritten against it** — every item 1–33 has a status; my from-scratch reconstruction had MISSED #25 azimuthal-mean structure, #7 wind radii, #21 B2 archive and #10 HGEFS entirely (the real cost of the drift, now recovered). **#26 + #4 LIVE-VERIFIED on cycle 2026080306**: 12w carries all 43 shear hours (params correct, values sane), 94w 25 (invest — honest degradation). **#27 LIVE**: 396 blocks vs 2,838 frames = 14.0% measured on the real cycle, and a ranged CDN read is BYTE-IDENTICAL to the per-frame PNG (1,422,668 bytes) — the flip criterion is met; the PNG-include drop waits a few days for cached frontends to age out. **Spec-alignment on #27**: one real divergence found and fixed — the spec caps block growth at 8 (1,2,4,8,8,…) to bound progressive-publish delay to ~18 min; mine doubled unbounded. Aligned (default cap 8 → 18.6%/row, uncapped mode retained), because the delay bound is what makes the design PORT: **a $448/16-day Cloudflare bill just landed (98.2M Class A, ~$1,350/mo run rate) and it is the SATELLITE tile pipeline — same PUT disease, same cure**; the container core was refactored into a domain-agnostic write_members/plan_blocks layer that knows nothing about forecast hours, ready for the tsr port. **Sequencing constraints audited**: (1) #13-before-#27 cost ~NOTHING — the paintball shipped as VECTORS (single-digit JSON writes/day), no member rasters ever existed, containers absorb nothing; (2) telemetry-before-scheduler is intact — #30 unbuilt — but the client counters aggregate NOWHERE without the collector endpoint, making that Andrew decision the only thing gating a phase. **Both needs-andrew decisions written in plain language in MODELS-SPEC-STATUS.md** (A: telemetry collector — worker/skip; B: guidance chart colors — keep/switch). Board: stale ships-diagnostics-panel duplicate retired; tar-container item updated with live numbers. All suites green after the cap-8 test updates._

---

_Last update: 2026-08-03 ~11:0x UTC — **#27 CONTAINER PUBLISHING BUILT AND LANDED (dual-publish transition), measured not projected.** **Backend**: build_cycle packs every rendered row into geometric tar blocks (hafs_container.row_container_plan) beside the PNGs; storms[].blocks carries per-block fxx lists + exact member byte offsets; manifest.containers declares the read path; workflow syncs tars with their own content-type and filter-scoped --delete. **Frontend**: _frameUrl resolves container-covered frames to a stable block+range cache key; the loader does fetch(Range) -> blob -> objectURL with decode-gating unchanged, blob URLs revoked per selection, and ANY ranged failure falls back to the per-frame PNG (dual-published) - proven in the harness with a 404ing tar. **Verified end-to-end in a real browser**: 47/47 harness assertions - block range key, blob display, range-sliced member decodes as the exact 1x1 PNG (offset proof), second block, fallback, zero page errors; builder glue proven through build_cycle with a file-writing fake pool (blocks cover exactly the rendered hours, tars on disk, manifest JSON-serializable); CORS/Range probed LIVE from triple-a-tropics.com against the real CDN (206, exact bytes). **Measured write math on the real catalog: 990/7,095 = 14.0%** (spec ~17%); spec's $374->$71/mo not derivable from today's catalog (R2 Class A $3.83->$0.53) - the ratio is what matters at 50-model scale. **Salvage constraint honoured by construction** (stated in hafs_container's docstring): plans operate on frames that EXIST; f000 IS a complete tar; deadline salvage = complete leading blocks + one short trailing block. **Rollout**: dual-publish now; the PNG-include drop in update-hafs.yml (the actual write saving) is a one-line flip after the first live container cycle verifies. hafs_render 0.15.0. **STILL PENDING THIS SESSION: first LIVE shear numbers** (the 12:53Z cron is the first to render a NEW cycle with the landed #26 pass; watcher armed) - #26/#4 are shipped on the board, live-value confirmation is the remaining checkbox._

---

_Last update: 2026-08-03 ~10:2x UTC — **0.13.3 PROVEN IN PRODUCTION (first clean cycle in 5 days: run 30795807983 SUCCESS, 2h07m, full 2838/2838, `expected` present) → shear-26-4-held LANDED: #26 + #4 SHIPPED.** The first post-fix run did exactly what the root-cause diagnosis predicted: no stall of any kind, full-speed ingest, complete cycle 2026080300 published 10:06:48Z. The five-day outage was max_tasks_per_child start to finish. **Landed the held union** (shear_diag.py + builder shear pass + manifest storms[].shear + frontend shear-relative overlay + harness; hafs_render 0.14.0): the next cron cycle publishes the first LIVE shear numbers (watcher armed to verify live values against the SHIPS-comparison expectations before calling it done-done). Roadmap: both items shipped 2026-08-03. Branch shear-26-4-held deleted (content on main). **#27 underway on the now-unified tree**: hafs_container.py (geometric planner + offset-verified tar writer + range index) landed with 9 tests; measured write reduction on the real 2026072818 catalog = 990/7095 = **14.0% of baseline** (spec projected ~17%; spec's dollar figures not derivable from today's catalog — R2 Class A $3.83→$0.53/mo — the ratio is what scales to 50 models). Salvage constraint honoured BY CONSTRUCTION: plan_blocks operates on frames that EXIST (f000 is a complete tar of one; a deadline salvage plans complete leading blocks + one short trailing block), reads stay per-frame via manifest byte offsets + HTTP Range. #28 reserved as (fxx, kind) members. Remaining #27 work: builder wiring + manifest blocks + frontend ranged reader + workflow sync. **Spec-drift audit pinned at /workspaces/MODELS-SPEC-STATUS.md** (items 10, 12–16, 19–25 definitions LOST to compaction — Andrew: re-drop the canonical spec there)._

---

_Last update: 2026-08-03 ~05:2x UTC — **ROOT CAUSE FOUND, REPRODUCED, AND KILLED: THE HAFS STALLS WERE NEVER THE NETWORK — max_tasks_per_child ZOMBIFIES THE POOL AT THE FIRST WORKER RECYCLE.** The forensics did their job on incident #5 (run 30764256854): at the live wedge the process tree showed **the ingest workers DID NOT EXIST** — no python children, no subprocess, nothing blocked in any fetch — the builder alone in futex_do_wait on futures that could never resolve, `ex._processes` EMPTY (which also retro-explains incident #4's "SIGTERM-immune orphans": the kill loop iterated an empty dict; nothing was ever signalled). **Local reproduction, 3/3, deterministic**: stock CPython 3.11.9 AND 3.12.1, ProcessPoolExecutor(max_workers=2, max_tasks_per_child=12), 80 memory-churning no-op tasks — completions stop at EXACTLY 24 (the first recycle boundary = width x N) and never resume. No Herbie, no GRIB, no network. Every incident's wedge-at-18–24-completions signature is this boundary, the "fresh worker Herbie banner then silence" is the spawn-recycled worker, and the parameter landed **2026-07-29 in the ingest memory rework (021bcd0b)** — the exact day the incidents began (incident #3's "same code" isolation missed it). Both Python-level timeouts and SIGTERM were structurally irrelevant: there was never a blocked call to interrupt. **THE FIX (hafs_render 0.13.3, on main)**: recycling moved to the POOL level — _run_pool slices the job list into chunks of (current width) x N and runs each in a FRESH executor (fork, the pre-rework regime that was stable for weeks); identical per-worker RSS bound (~2.15 GB at N=12, budget model unchanged), max_tasks_per_child BANNED with the repro documented at the constant; chunk size tracks the halved width so the bound holds in the OOM-degraded regime; the stage deadline drains not-yet-attempted chunks so the ledger stays complete. **Proven on the affected interpreter**: the real _run_pool completes 80/80 past the boundary on the same 3.11.9 that wedged raw, and two new regression tests run the chunked path + deadline drain in the suite (5 teardown tests total, full suite green, validated against the constructed main tree). **ALSO: the post-manifest sit is covered by a C-level self-rescue** — incident #5 confirmed the commit point WORKED (23:56 deadline salvage → 24 frames → 288 renders → "wrote manifest.json cycle 2026080212" at 23:58:57) and then the builder process sat alive 2.5 h in the epilogue's vicinity, so the workflow's R2 publish steps never ran and the salvage was lost; faulthandler.dump_traceback_later(120, exit=True) is now armed after main() returns — immune to Python locks/GC by construction, dumps EVERY thread's stack if it ever fires, and _exits so the publish steps always run. Forensics upgraded: parent thread dump (faulthandler) + executor internals (broken/shutdown flags, process map, pending count, thread names) — that introspection would have named this bug on day one. **NEXT**: the currently-running cron (30778707120, pre-fix code) dies ~07:5x; the next cron picks up 0.13.3 → expect either a FULL cycle (the "egress stall" never existed, so ingest should just run: the codespace does these frames at 14–17 s each) or worst-case a deadline salvage that now PUBLISHES. On fresh manifest: land shear-26-4-held (#26+#4, flips shadow→shipped), then the #27 tar-container assessment._

---

_Last update: 2026-08-01 ~05:0x UTC — **TEARDOWN FIX SHIPPED (incident #4 defect 2) + STALL FORENSICS ARMED (defect 1 instrumentation).** Per instruction, no third Python-level timeout anywhere. **(1) THE COMMIT-POINT RESTRUCTURE (hafs_render 0.13.2, on main)**: stage-deadline path now **p.kill() + reap** instead of terminate() — SIGKILL needs no interpreter participation, and dead readers break the call-queue pipe so the feeder thread exits and every downstream join returns (the incident-#4 chain: SIGTERM-immune workers → open pipe → blocked feeder → multiprocessing's UNBOUNDED Queue._finalize_join fired from cyclic GC in the MAIN thread at some later allocation → hung one second short of the manifest write, 288 salvaged PNGs lost). The manifest write is documented as THE COMMIT POINT with nothing blockable ahead of it, and the entrypoint ends in **os._exit(rc)** after logging.shutdown()+flush, so no join/atexit/finaliser can hold a finished build hostage. **Proven behaviourally** (hafs_render/tests/test_pool_teardown.py): a pool of deliberately unkillable workers — SIGTERM+SIGALRM masked via pthread_sigmask, then wedged in a single long C call, the production signature — is salvaged at a 6 s test deadline, the workers actually DIE, completed tasks record ok, wedged record as stragglers, and the post-stage gc.collect() (the exact production kill-shot) returns instantly. **(2) STALL FORENSICS (capture-only, fires while the wedge is LIVE)**: a daemon watcher in the deadline stage; if completions stop for 8 min (HAFS_STALL_FORENSICS_S) it captures ONCE — full process tree (ps, descendants of the builder — worker CHILDREN are what decides subprocess-vs-in-process), and per worker /proc State/wchan/syscall, kernel stack via sudo, py-spy dump (plain then sudo; workflow now installs py-spy, optional). Every capture is individually guarded and time-bounded; the deadline still does the salvaging. The next wedge self-documents WHERE it blocks — the fetch-path bound gets chosen from that evidence (native-layer timeout vs killable subprocess), not guessed. **Committed to main surgically**: the worktree carries the held shear work in the same file, so main got a constructed blob = HEAD + teardown/forensics hunks only (verified in a scratch worktree: zero shear refs, full suite green); the shear work remains on shear-26-4-held and the union worktree is what lands when the gate clears. **NEXT**: watcher armed on the manifest — the next cron (crons 53 0,6,12,18 + backup) runs the fixed code; worst case it publishes a salvage at deadline+~15 min. When generated_at goes fresh: land shear-26-4-held (commit the union worktree files), flip #26/#4 shadow→shipped. If it wedges again: the log now contains the stacks._

---

_Last update: 2026-08-01 ~04:0x UTC — **INCIDENT #3 RECOVERY RUN FAILED (now #4) — TWO OF THREE 0.13.1 LAYERS DEAD ON ARRIVAL, THE THIRD WORKED AND EXPOSED A NEW POST-RENDER HANG. SHEAR BUILD (#26+#4) FINISHED + VERIFIED BUT HELD AT THE GATE, on branch `shear-26-4-held`.** **THE RUN (30661097361, first with 0.13.1)**: queued 2 h, step started 21:53Z; the egress stall recurred at the SAME coordinates as incident #3 — 23/258 ingested in ~9 min, then TOTAL worker silence for 3 h 10 m. **Layer 1 (socket 120 s) and layer 2 (SIGALRM 300 s/attempt) never fired**: zero retry logs, zero timeout tracebacks across the whole window, and at job teardown the runner terminated **two live python orphans** — the stalled ingest workers, still alive 2.5 h after the deadline handler SIGTERM'd them. A block that ignores a 120 s socket timeout, a 300 s alarm AND SIGTERM is stuck below Python's signal delivery (uninterruptible syscall, masked signals, or wedged inside a child the worker is waiting on — Herbie's curl path is a suspect: the last worker output is a fresh worker's herbie-import banner at 22:03:15Z). Next session needs worker-side forensics (py-spy dump in CI on wedge, or move the fetch into a `curl --max-time` subprocess, or SIGKILL-based recycling). **Layer 3 (200-min stage deadline) WORKED EXACTLY AS DESIGNED**: fired at 12000 s on the dot (01:13:54Z), logged all 234 stragglers, salvaged the 24 ingested frames, and render completed cleanly — 288 PNGs, 0 failed, by 01:16:22Z. **THEN THE NEW DEFECT**: after "rendered 288 ok" the builder never printed its NEXT log line (the local manifest write in main(), pure code ~1 s away) and never exited — 2 h 27 m of silence until the 350-min kill. The salvage was lost; nothing published; manifest still 2026-07-29T05:32:53Z. Leading hypothesis: main thread blocked on terminated-ingest-pool residue (unkillable workers + executor/queue machinery; the SIGTERM'd-but-immortal workers are the same disease as layers 1-2). Fix candidates, in order: `p.kill()` + reap with timeout instead of `terminate()`; write + publish the manifest BEFORE any pool-teardown exposure; `os._exit(0)` after a successful main() so no join can hold a finished build hostage; and re-budget the deadline so salvage + publish always fit under 350 min. **THE ESCALATING MONITOR IS DOING ITS JOB**: HAFS re-alarms every 6 h until recovery — no silent window this time. **MEANWHILE, THE SESSION'S BUILD (spec #26 vortex-removed shear + #4 shear-relative view) IS COMPLETE AND FULLY VERIFIED, HELD UNCOMMITTED-to-main PER THE GATE** ("nothing gets built on top until the pipeline has had a clean run"), preserved on pushed branch **`shear-26-4-held`**: (1) **prerequisite resolved — NO ingest change needed**: the v5+ field cache already stores the 200-850 shear VECTOR per parent frame (`shru_/shrv_200_850`, kt), and the azimuthal-mean operator is linear so removal on the difference field == per-level removal then differencing — zero CACHE_VERSION bump; (2) `hafs_render/shear_diag.py` — SHIPS-SHRD-construction removal (azimuthal-mean tangential+radial about the MODEL'S OWN trak.atcfunix fix, 0-500 km, parent domain), method/layer/radius as named parameters, Helmholtz a raising seam, antimeridian-normalised before any distance/angle, coverage guard (<0.6 → publish nothing); (3) builder post-ingest pass publishes `storms[].shear` = params + per-(model,fhour) `{kt, hdg, naive_kt, naive_hdg}` — no vortex fix → no number; (4) frontend toggle on the storm view: raster stays north-up, SVG overlay rotates the shear arrow + DR/DL/UR/UL quadrant frame (pure rotation = hemisphere-identical; chip states downshear-LEFT NH / downshear-RIGHT SH, both ways; TAT serves SHEM), `shear=1` replaceState-only, S key in the expert layer, honest per-hour degradation. **VERIFIED WITH NUMBERS**: vs SHIPS same storm+cycle (EP07 GENEVIEVE 2026-07-31 18Z, n=8 taus): removed-minus-SHRD mean **−1.5 kt**, range [−4.0, +2.7] — same ballpark from INDEPENDENT model fields (HAFS vs GFS); SHTD agrees to ~14° median **after a +180 flip — SHIPS SHEAR DIR is the FROM convention, ours is TOWARD** (documented in module + manifest params). DOLPHIN (12W, 113–133 kt) computed smoothly on a real continuous frame spanning lon 103.7→204.5 (crosses 180); synthetic translation-invariance test pins the centred-at-179.8 case. Contamination-grows-with-Vmax proven where it physically lives: on CLIPPED discs naive error grows 0.5→6.3 kt as the vortex triples while the removed value stays FLAT (intensity-independent = the vortex is gone); on full centred discs removed==naive by symmetry (the disc-average self-cancels an axisymmetric vortex — expected mathematics, stated, not hidden). SHEM: mirrored NH/SH unit tests recover IDENTICAL shear vectors. 14 new module tests, 76/76 hafs_render suite, 40/40 browser-harness assertions, overlay screenshot-verified over a real published frame. **Design call (documented, per working agreement)**: the spec's "overlays on the unrotated raster" reading — rotating the quadrant frame, not the PNG (a rotated raster garbles titles/colorbars/ticks; house style forbids the mess). **Also landed on main**: root-suite `fake_pool` stub taught `stage_deadline_s` (missed mirror from the 0.13.1 change — 4 planning tests were erroring). **Roadmap**: both items added as **shadow** (built, held at the gate) — flip to shipped when the pipeline has a clean run and the branch lands. **Pre-existing, not mine (stash-verified)**: test_enscenters (4), test_tiled_viewer_playback (1), knackwx/jtwc-prose loader errors. **NEXT SESSION, in order**: (1) the two remaining hang defects above — that is the actual blocker for everything; (2) land `shear-26-4-held` once a run publishes fresh (verify `expected` fields + shear block appear); (3) cyclolab analogs still stale (Jul 16) and alarming — needs its own diagnosis._

---

_Last update: 2026-07-31 ~21:4x UTC — **HAFS STALENESS INCIDENT #3 — DIAGNOSED (not restarted), HANG-PROOFED, ALERTING RE-ARMED + ESCALATING.** **The cycle was NOT over budget**: the failing cycles are SMALLER than the last one that fit (258 ingests/4,257 renders vs 7,095 frames). Every Actions run since Jul 29 ~02Z wedged identically — ingest hit ~23/258 in ~7 min, then the log went SILENT for 5h43m until `timeout-minutes: 350` killed the job (GitHub reports the killed step "cancelled") with ZERO frames rendered. Isolation was airtight: dep-diff between last-good and first-hang runs is EMPTY, same runner image, same code — and the EXACT stall-window frames ingest in 14–17 s from this codespace with production parameters. **Cause: the Actions-egress path to noaa-nws-hafs-pds stalls; our defect: the fetch path had NO timeout of any kind**, so one stalled read wedged a worker forever (width 2 → two stalls froze the stage). **The watchdog was NOT livelocked — it worked**: `workflow_already_running` saw a run perpetually in flight and correctly declined every 20 min for three days; when the runs themselves are the failure, a dispatch guard is a spectator (its dispatches would have hung identically). **Boxes irrelevant, confirmed three ways**: HAFS worker lane profile-gated OFF, no hafs lane in fleet.yml, manifest carries only cron-signature writes (last 2026-07-29T05:32:53Z). **FIX (hafs_render 0.13.1)**: socket default timeout (120 s) in ingest workers + SIGALRM per attempt (300 s; disarmed before the retry sleep and on every exit path) + a whole-stage deadline (200 min) that TERMINATES a blackholed pool — caught INSIDE the with-block because unwinding through `__exit__` would join the wedged workers forever — then renders and PUBLISHES what it has. Verified behaviorally: a pool of deliberately wedged workers fails fast, respects the deadline, records every task, continues. **WHY THREE INCIDENTS RAN SILENT**: freshness-monitor's HAFS entry still carried incident #1's known-down silencer ("gated off via RENDER_HAFS_ON_CRON; manifest fix in flight") — known-down never alarms. Removed (any future mute must state its re-arm condition), and red-once alerting is now **ESCALATING: still-stale products re-alarm every 6 h until recovery** (state carried in the rollup). Also repointed the sst-animations probe off the mp4-artifacts branch (retired Jul 19 → the dead-branch probe read permanently stale). **The re-armed monitor's FIRST run surfaced a FOURTH silent staleness: cyclolab analogs — last write 2026-07-16, update-analogs runs FAILING** — left alarming on purpose; needs its own diagnosis. **RECOVERY TIMELINE**: gh cancel/dispatch are 403 from here, so the zombie (old code) dies ~21:52Z, the 00:53Z cron (created post-push → carries the fix) runs next: full cycle ~2.5–4 h if the network path has healed, else partial publish at the 200-min deadline — fresh manifest expected ~04–06Z Aug 1 either way, and the monitor now nags red every 6 h until it happens._

---

_Last update: 2026-07-31 ~20:1x UTC — **MODELS INTERACTION BATCH (spec 9/11/17/18) — all four live on /models/, each browser-proven.** **(17) FIVE-STATE AVAILABILITY**: the manifest now carries per-pair EXPECTED hours (captured at render planning BEFORE the progressive-subset narrowing — the worker's per-pass ledger names what to render now, not what the pair will eventually have), and the hour strip distinguishes lit / pending (expected, cycle building — wait ~90 s, dashed) / **unavailable** (expected, cycle COMPLETE, never rendered — failed, struck through) / unscheduled (upstream never posted it, near-invisible). Legacy manifests degrade to the old two-state strip. Harness-proven incl. the poll-merge flip pending→unavailable. **(18) ORTHOGONAL URLs**: run/storm/model/domain/product/fxx/mode as independent query params; **history policy proven in a real browser** — scrubbing every hour grows history by ZERO entries (replaceState debounced 250 ms), selection changes push exactly one entry each and only from the user-click path (the 45 s poll's regrow calls the same _select* functions but never the click handlers, so it structurally cannot spam), back/forward restore silently, and an expired shared run falls back to the current run, KEEPS the rest of the link, and says so in a toast. CycloLab embeds never rewrite their host URL. **(11) SMART DEFAULT**: land on a storm when storms are active, ordering stated and deterministic — named before invests always, then strongest by season-peak from the live feed (fetched parallel to the manifest, 1.2 s race, never delays boot), then deck number, then id. **Verified live: landed on 12w DOLPHIN (feed 150 kt) over 07e GENEVIEVE (140) and over the manifest's first-listed storm.** URL storm still beats it. **(9) EXPERT KEYBOARD LAYER**: ←/→ hour, **↑/↓ RUN TREND — same VALID time across successive inits** (valid = init + fxx; the 6-h-older run lands on fxx+6, narrated in a toast; a missing exact valid time shows nearest AND says so), Space, Home/End, Esc, ? sheet. **WCAG 2.1.4**: the character keys (Space, ?) sit behind a persisted toggle in the sheet — proven to actually disable Space while arrows keep working — and a visible ⌨ Shortcuts button means the sheet can't be locked away. Keys fire only while the viewer is on-screen, never in form controls. **Live run-trend verified on real runs: F24@2026072818 → F30@2026072812, toast "Run Jul 28 12Z · same valid time (F030)".** 30-assertion browser harness (tests/models_url_harness.cjs, serves the repo over local HTTP + synthetic 2-cycle manifest) + 5 new node-harness tests; zero page errors live. **OPERATIONAL FLAG**: the live stale banner correctly shows the HAFS runs are ~3.3 days old — the boxes are still dark and update-hafs cron runs have been cancelling (timeouts); one is in_progress since 14:10Z and should refresh the manifest when it lands. It predates the `expected` commit, so the five-state strip's full distinctions activate one render later; until then every tick is honestly `lit`/`pending`._

---

_Last update: 2026-07-31 ~16:2x UTC — **PNG-8 (premise falsified, shipped where honest) + THE VERIFICATION SCOREBOARD (headline item, live).** **(1) PNG-8.** The spec's premise — bit-exact on 100% of colortable fill pixels — was MEASURED before anything was written, and it does not survive contact with the real colortables: every continuous ramp carries 257–871 LUT entries and EVERY entry appears in real frames (a smooth field sweeps its whole ramp), so a 256-entry palette cannot hold them; quantizing would nearest-map genuine fills, the exact "probably identical" outcome the build was told to refuse. Shipped structurally scoped instead: eligibility = complete colortable fits with overlay room (MAX_CT=200) → today that is reflectivity (14 colours, the one discrete .pal product); any future discrete product inherits, any palette growth degrades to PNG-24 at the gate. **Every emitted PNG-8 is decoded back and verified bit-exact on every colortable pixel BEFORE writing** — per frame, not sampled — with PNG-24 fallback on any surprise. Measured on six real production refl frames: **32.2% of PNG-24 bytes aggregate (spec predicted 31.8), decode 77→12 ms (6.4×)**, ~1.1 s/frame transcode INCLUDING verification (the spec's "4× cheaper encode" applies to a direct-raster path not taken — running the identical savefig raster through a verified transcode is what makes the swap provably encoding-only), AA text/contours visually indistinguishable (2–3.5% of pixels, mean Δchannel ~12, all AA blends). Fleet + cron pick it up on next pull/run. **(2) SCOREBOARD (spec #29, impact 10) — live on the CycloLab Scores tab.** Engine rules enforced in code, each with a test: homogeneity as a HARD filter (drop models least-coverage-first — recorded and shown — never relax; short lead times omitted with their n printed); **n= + storm count under every point**; CIs **block-bootstrap over STORMS** (single-storm sample → "no interval", not a fake one; seeded/deterministic); **early/late never pooled** (separate panels, baseline anchors both); **OCD5 protected** from the coverage rule with skill% against it everywhere. Caveats ride IN the document so a viewer change can't drop them: **provisional truth** (the b-deck self-corrects — the Dolphin 140→150/909 revision we observed is cited) and **the filtered deck** (ECMWF/UKMET aids CANNOT be scored from public data — said plainly, not silently omitted). NHC basins only, structurally: a JTWC storm page renders the explanation and doesn't even fetch. **Engine catch from real data: NNIC** (neural-net INTENSITY consensus) carries positions that are not track forecasts — 628 nm @120 h vs TVCN's 82 in the same homogeneous sample — its CONSENSUS kind admitted it to track panels; intensity-consensus aids now track-excluded, intensity-eligible. **First real numbers (EP 2026, 7 storms, 864 cases, all 7 taus)**: OFCL 18.7 nm @12 h → 59.9 @120 h; consensus 82–92; hurricane models 118–145; OCD5 32→269. Textbook shapes = sanity check passing. AL honestly shows only to 48 h (2 storms); CP nothing (1 case). **WORKFLOW gains a push trigger** (guidance/** → immediate republish; also the only self-service publish while both boxes stay UNREACHABLE — still dark this session) + verification build + own R2 prefix. Live-verified on GENEVIEVE (full board, zero page errors) and DOLPHIN (structural note, zero errors). hafs_render 95 tests; 18 new engine tests; roadmap items for both builds._
...
- **2026-09-02 20:2x · Step 2 deploy on box1 once steal < ~50 % (code is on tsr main @72cb9d9; .env keys already set).**
  From a tsr checkout: `scripts/fleet.sh deploy box1` (recreates all four
  lanes governed; the stopped browse lanes come back — `docker stop` them
  again if you want them one at a time), then on box1 from /root/tsr-s2:
  `docker compose -p tat-render -f docker-compose.render.yml -f docker-compose.render.override.yml build render`
  and `... up -d --no-deps --no-build render` (verify exactly ONE render
  container). meso-render/-cold and their mem_limit are the
  tat-satellite-render tree owner's; s1 waits for the RETAIN_H/purge decision.
  The agent will do all of this itself when the cap lifts unless told not to.

- **2026-09-02 · box1 under Hostinger's CPU limiter — hPanel first, ticket second, weekly removal LAST.**
  (1) hPanel srv1739889: read the CPU-limit notice, Server Usage, and the
  bandwidth meter (August total, September to date; readings at Aug 24 ~20:xx
  and Aug 30 13:09 UTC if history is kept); same for box2 (72.62.97.220) so
  the meters can be compared; screenshot. (2) Give the go for Step 1 in
  `INCIDENT-2026-09-02-box1-cpu-cap.md` (restart meso-render + meso-render-cold
  one at a time, `docker stop` tat-s2-g19fd-emit-cron-1, tat-s2-conus-emit-cron-1,
  tat-s1-s1-ingest-goes18-1); it is reversible and touches nothing under
  /root/tat-satellite-render. (3) Do NOT click hPanel's once-per-week "remove
  CPU limit" before Step 1 has run — it burns the removal on a full-speed
  catch-up that re-enters the 25 %/hour limiter. (4) Ticket to Hostinger with
  the questions in the report (limiter since when/threshold/auto-lift; shaper
  tiers at ~20 TB and ~22-25 TB vs the published 32 TB; why box2 was never
  shaped at ~24-25 TB). (5) s1 shadow backlog (4,570 SQS slots 22-29 h old):
  purge or approve a smaller S1_RETAIN_H before the two exited lanes start.
  (6) The meso-render mem_limit + queue-deadline edits live in
  /root/tat-satellite-render's dirty tree: release it or assign its owner.
  (7) September ingest strategy (dedupe across consumers / drop products /
  third box / plan with written limits) — moving lanes to box2 only spreads the
  throttle.

- **2026-08-26 20:19Z — sandbox-api CPU hotfix DEPLOYED (Andrew's instruction), version a0b94320:** native-base64 patch + explicit `[limits] cpu_ms = 30000` in one `wrangler deploy` (`workers/sandbox-api-hotfix/deploy.sh`); bindings/secret/custom domain verified, health 200. First 180 s tail: 29/29 ok, 0 exceededCpu; checkpoint PUT median CPU 129 → 24.5 ms, a 153 ms request completed. **Verdict 22:18Z: zero.** 1,867 requests in the two hours after the deploy, 0 `exceededResources` (vs 10–19%/hour before); requests at 300–440 ms CPU complete. The missing `[limits]` block was the whole problem; the patch is insurance (median write CPU 129 → 25 ms). Hourly table in COST-AUDIT-2026-08.md.
- **2026-08-25 (cost audit, after A/B/C):** (1) the kill-switch deploy is still queued exactly as written above (merge tsr `cost-breaker` + TAT `cost-killswitch`, then `fleet.sh deploy all`; TAT's copy auto-deploys to the overlay pollers on merge). (2) If the `s2-manifest-coverage` saving on the slow lanes (hwfd, g19fd) is wanted, raise `S2_COVER_VERIFY_S` (e.g. 7200) in the box `.env` for those lanes and recreate them — a knob, not code. (3) sandbox-api: if single-digit `exceededCpu` kills at 50–100 ms persist after 2026-08-26, add `[limits] cpu_ms = 30000` to its wrangler config and redeploy; consider `crypto.subtle.digest` for the 1 MB checkpoint hashes. (4) `prune-hafs.yml` applies daily at 05:31Z from now on; the 2026-08-26 GraphQL daily total is the definitive before/after for B and C (LIST/day was 383K on 08-24/25).

- **2026-08-25 · Cost lockdown: arm the breaker only AFTER the box kill switch is deployed.**
  (1) Deploy the switch (recreates lanes, hence held): merge tsr `cost-breaker`
  (3b9b02c) → main and TAT `cost-killswitch` (cc02f38b) → main (merging TAT IS
  deploying for the overlays pollers, which hard-reset to main every tick),
  then from a tsr checkout `scripts/fleet.sh deploy all` (rebuilds images; the
  boxes are also 10 tsr commits behind main, which this pulls along). Verify:
  `scripts/fleet.sh writes off box2` → every lane `guard: present` →
  `scripts/fleet.sh writes on box2`. ~30–60 s per box. (2) After ≥7 days of
  alert-only (from 2026-08-25 16:00Z): if `would_have_tripped_count` is 0 on
  normal days and no `breaker` issues exist, `scripts/fleet.sh breaker arm`
  (admin key is `TAT_BREAKER_ADMIN_KEY` in box1 `/root/tsr-s2/.env`). `reset`
  is the only way back after a trip; `disarm` does not re-enable writes.
  (3) Token hygiene: create a Cloudflare API token with Account Analytics:Read
  only and `printf '%s' "$TOKEN" | npx wrangler secret put CF_GRAPHQL_TOKEN -c
  workers/r2-breaker.toml` — the Worker currently holds a copy of the
  Codespace deploy token for its GraphQL read. (4) Phase 2 decisions A–E in
  `COST-AUDIT-2026-08.md`; A (`models/hafs` 14-day retention prune) is the
  only compounding cost line and a one-file change.

- **2026-08-22 · Floater loop lags meso (single shared `render` worker); reduce floater render capacity on box1.**
  The storm floater loop runs ~22-50 min behind real time (oscillating with
  its sweep cycle) while the mesoscale panel is ~2 min behind. Root cause: the
  floater, intensity and guidance pollers all funnel through ONE shared
  `render` container (`uvicorn app:app --workers 1`, tsr
  docker-compose.render.yml), rate-limited across 9 active storms x up to 6
  bands; meso rides its OWN dedicated render container, so it stays current.
  Box1 is NOT the bottleneck (load ~0.38/core). Not fixable from the Codespace:
  SSH egress to the box IPs is filtered here (`:22`/`:443` closed) and
  `gh workflow run` 403s (token has no Actions write). Options, cheapest first:
  bump the render worker count (`--workers 2`+ on the `render` service, box1
  has headroom), or give the floater poller its own render container like meso.
  To inspect first: `ssh -i ~/.ssh/tat_box root@2.25.183.231
  'docker logs --since 30m floater-poller; docker logs --since 10m render'`.
  The monitor now catches a genuine FREEZE (>90 min) via the new
  floater-frame-recency discriminator, but the chronic lag is this capacity
  change. NOT urgent — frames do flow; it is a latency/UX issue, not an outage.
