...
Renders one MP4 per (region × product) for four families:
...
= 18 regions × 4 families = 72 clips. Raw fields only; anomaly /
records / GMR variants are out of scope at this step.
...
def prefetch_armor3d(missing_dates: list[dt.date],
    ...
    for chunk_start, chunk_end in chunks:
        ...
        try:
            _materialize_armor3d_chunk(chunk_path, log)
        finally:
...
def _materialize_armor3d_chunk(chunk_path: Path, log: str) -> None:
        ...
        with xr.open_dataset(chunk_path) as ds:
            ...
            for ts_idx in range(len(time_vals)):
                ...
                try:
                    ...
                    print(f"{log}   ! ARMOR3D {d} write failed: {exc}",
                          file=sys.stderr)
                    continue
...
def _missing_dates_for_source(source: str, regions: list[str],
    ...
    OPeNDAP fetch."""
    families = _families_for_source(source)
    out: list[dt.date] = []
    for d in dates:
        for fam in families:
...
def _render_all_frames(dates: list[dt.date], regions: list[str],
    ...
    t0 = time.time()
    sources = sorted({cfg["source"] for cfg in FAMILIES.values()})

    total_targets = len(dates) * len(regions) * len(FAMILIES)
    progressed = 0
...
def main(argv=None):
    ...
    if not args.skip_armor3d:
        ...
        prefetch_armor3d(armor3d_missing, log)
    ...
    for family in families:
        clips, build_dir = _encode_family(family, regions, end_date, log)
        _write_family_manifest(family, clips, regions, end_date, build_dir)
